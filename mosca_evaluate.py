...
def render_test(
    ...
    for i in tqdm(range(len(test_camera_tid))):
        ...
        test_ret.append(rgb)
        if save_dir:
            imageio.imwrite(osp.join(save_dir, f"{fn_list[i]}.png"), (rgb*255).astype(np.uint8)) # MARK: save image
...
def render_test_tto(
    ...
    # first align the camera
    with torch.no_grad():
        solved_cam_T_wi = torch.stack([cams.T_wc(i) for i in range(cams.T)], 0) # T_train,4,4 NOTE c2w
        aligned_test_camera_T_wi = align_ate_c2b_use_a2b(
            traj_a=train_camera_T_wi,
            traj_b=solved_cam_T_wi.detach().cpu(),
            traj_c=test_camera_T_wi,
    ...
    test_ret = []
    solved_pose_list = []
    for i in tqdm(range(len(test_camera_tid))): # nvidia_marbles: Balloon1 192
        if initialize_from_previous_camera and i == 0:
            step_factor = initialize_from_previous_step_factor # 10
            lr_factor = 1.0
        else:
            step_factor = 1
            lr_factor = initialize_from_previous_lr_factor # 0.1

        working_t = test_camera_tid[i]
        # load gt rgb and mask
        gt_rgb = imageio.imread(osp.join(gt_rgb_dir, f"{fn_list[i]}.png")) / 255.0 # H,W,3
        gt_rgb = gt_rgb[..., :3]
        if gt_mask_dir is None: # None
            gt_mask = np.ones_like(gt_rgb[..., 0]) # H,W
        else:
        ...
        # Fine-tuning aligned_test_camera
        T_cw_init = torch.linalg.inv(aligned_test_camera_T_wi[i]).to(device) # 4,4 NOTE w2c
        ...
        optimizer = optimizer_type(
            [
                {"params": t_init, "lr": lr_p * lr_factor}, # 0.003 * lr_factor
                {"params": q_init, "lr": lr_q * lr_factor}, # 0.003 * lr_factor
            ]
        )
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=tto_steps * step_factor - decay_start, # 30 * 10 - 15
            eta_min=lr_final * lr_factor, # 0.0001 * lr_factor
        )
        ...
        for _step in range(tto_steps * step_factor): # 30*10 or 30*1
            optimizer.zero_grad()
            _T_cw = torch.cat([quaternion_to_matrix(q_init), t_init[:, None]], 1) # 3,4 NOTE w2c
            T_cw = torch.cat([_T_cw, T_bottom[None]], 0) # 4,4
            render_dict = render(gs5, H, W, cam_K, T_cw=T_cw) # NOTE w2c
            pred_rgb = render_dict["rgb"].permute(1, 2, 0) # H,W,3
            rendered_mask = render_dict["alpha"].squeeze(-1).squeeze(0) > fg_mask_th # H,W
        ...
        solved_T_cw = torch.cat([quaternion_to_matrix(q_init), t_init[:, None]], 1) # 3,4 NOTE w2c
        solved_T_cw = torch.cat([solved_T_cw, T_bottom[None]], 0) # 4,4
        solved_pose_list.append(solved_T_cw.detach().cpu().numpy()) # T,4,4
        with torch.no_grad():
            ...
            if optimizer_tion:
                ...
                render_dict = render(
                    [s_model(), d_model(working_t)], H, W, cam_K, T_cw=T_cw_init
                )
            rgb = render_dict["rgb"].permute(1, 2, 0).detach().cpu().numpy()
            rgb = np.clip(rgb, 0, 1)  # ! important
            test_ret.append(rgb)
        ...
        if initialize_from_previous_camera and i < len(test_camera_tid) - 1: # NOTE initialize_from_previous_camera: True
            aligned_test_camera_T_wi[i + 1] = torch.linalg.inv(solved_T_cw).to(
                aligned_test_camera_T_wi
            ) # NOTE Applies to stable test camera views
...
def test_main(
    ...
    data_root,
    device,
    tto_flag, # NOTE tto_test
    eval_also_dyncheck_non_masked=False,
    skip_test_gen=False,
    nvidia_mode=False,
    optimizer_tion=True, # True
    eval_background_covisible_mask=False, # False
):
    ...
    if dataset_mode == "iphone":
        if nvidia_mode: # nvidia marbles dataset
            (
            ...
            ) = load_nvidia_gt_poses(data_root, getattr(cfg, "t_subsample", 1))
            gt_dir = osp.join(data_root, "rgb/2x")
            gt_mask_dir=osp.join(data_root, "covisible/2x/val")
        else: # iphone
            (
                gt_training_cam_T_wi,
                gt_testing_cam_T_wi_list,
        ...
        # * cfg
        tto_steps = getattr(cfg, "tto_steps", 30) # 30
        decay_start = getattr(cfg, "tto_decay_start", 15) # 15
        lr_p = getattr(cfg, "tto_lr_p", 0.003) # 0.003
        lr_q = getattr(cfg, "tto_lr_q", 0.003) # 0.003
    ...
    elif dataset_mode == "nvidia": # nvidia rodynrf dataset
    ...
    eval_prefix = "tto_" if tto_flag else "" # iphone: True; nvidia: True
    
    if not skip_test_gen: # not False
        for test_i in range(len(gt_testing_cam_T_wi_list)): # test view 1, test view 2
            testing_fov = gt_testing_fov_list[test_i]
            testing_focal = 1.0 / np.tan(np.deg2rad(testing_fov) / 2.0)

            if tto_flag: # True test time optimization
                frames = render_test_tto(
                    ...
                    # optimizer_tion
                    optimizer_tion=optimizer_tion,
                )
                viz_frames = (np.stack(frames, 0)*255.).astype(np.uint8)
                imageio.mimsave(
                    osp.join(saved_dir, f"tto_test_cam{test_i}.gif"), viz_frames
                )
            else: # Fasle test time optimization
                frames = render_test(
                    H=H,
                    W=W,
                    cams=cams,
                ...
                viz_frames = (np.stack(frames, 0)*255.).astype(np.uint8)
                imageio.mimsave(osp.join(saved_dir, f"test_cam{test_i}.gif"), viz_frames)

    # * Test
    if dataset_mode == "iphone":
        eval_dycheck(
            save_dir=saved_dir, 
            gt_rgb_dir=gt_dir, # './data/iphone/apple/test_images'
            gt_mask_dir=gt_mask_dir, # './data/iphone/apple/test_covisible'
            pred_dir=osp.join(saved_dir, f"{eval_prefix}test"), 
            save_prefix=eval_prefix, # tto_ NOTE save_prefix for saving
            strict_eval_all_gt_flag=True,  # ! only support full len now!!
            eval_non_masked=eval_also_dyncheck_non_masked, # False
            eval_background_covisible_mask=eval_background_covisible_mask, # False
        )
        ...
        if data_root.endswith("/"):
            data_root = data_root[:-1]
        eval_nvidia_dir(
            gt_dir=gt_dir, # './eval_utils/nvidia_rodynrf_gt/Balloon1'
            pred_dir=osp.join(saved_dir, f"{eval_prefix}test"), 
...
def test_pck(saved_dir, gt_npz_fn, device, save_fn=None):
    ...
    ) # src: N_i,2; dst_gt: N_i,2; src_t: N_i; dst_t: N_i; img_wh: 2; ratio: scalar
...
def test_fps(saved_dir, rounds=1, device=torch.device("cuda:0")):
    ...
    viz = np.concatenate(viz, 1) # H,W*3,3 NOTE concatenate image
    imageio.imsave(osp.join(saved_dir, "fps_eval_samples.jpg"), (viz*255).astype(np.uint8)) # NOTE fps_eval_samples.jpg

    cnt = cams.T * rounds # NOTE iphone rounds: 1
    with torch.no_grad():
        start_t = time.time()
...
if __name__ == "__main__":
    ...
    test_main(
        ...
        tto_flag=True, # True
        eval_also_dyncheck_non_masked=False,
        skip_test_gen=True, # NOTE skip render and save test images
        nvidia_mode=args.nvidia_mode,
        optimizer_tion=True, # True
        eval_small_covisible_mask=True, # False
    )
    ...
    #     nvidia_mode=args.nvidia_mode,
    #     optimizer_tion=True, # True
    #     eval_small_covisible_mask=False, # False  
    # )
