...
def compute_static_ba(
    ...
    s_track = s_track[..., :2].clone() # T, N_static_track, 2
    s_track_valid_mask = s_track_valid_mask.clone() # T, N_static_track
    device = s_track.device
    if max_num_of_tracks < s_track.shape[1]: # avoid track is dense
        logging.info(
    ...
    homo_list, dep_list, rgb_list = prepare_track_homo_dep_rgb_buffers( # [T, N_static_track, 2]; [T, N_static_track]; [T, N_static_track, 3]
        s2d, s_track, s_track_valid_mask, torch.arange(s2d.T).to(device)
    )

    if viz_video_rgb is not None: # None
        logging.info(f"Viz BA points on each frame...")
        viz_frames = viz_ba_point(viz_video_rgb, s_track, s_track_valid_mask)
        imageio.mimsave(osp.join(log_dir, "BA_points.gif"), viz_frames)
    ...
    param_scale = torch.ones(cams.T).to(device) # NOTE scale
    param_scale.requires_grad_(True)
    param_dep_corr = torch.zeros_like(dep_list).clone() # NOTE correction factor
    param_dep_corr.requires_grad_(True)
    optim_list = cams.get_optimizable_list(lr_f=lr_cam_f, lr_q=lr_cam_q, lr_t=lr_cam_t) # focal, quaternion, translation; only q, t
    if lr_dep_s > 0: # iphone: 0.0, nvidia_rodynrf: 0.001
        optim_list.append(
            {"params": [param_scale], "lr": lr_dep_s, "name": "cam_scale"}
        )
    if lr_dep_c > 0: # iphone: 0.0, nvidia_rodynrf: 0.001
        optim_list.append(
            {"params": [param_dep_corr], "lr": lr_dep_c, "name": "dep_correction"}
        )
    optimizer = optimizer_class(optim_list) # optimizer_class = torch.optim.Adam
    scheduler = None
    s_track_valid_mask_w = s_track_valid_mask.float() # T, N_static_track
    s_track_valid_mask_w = s_track_valid_mask_w / s_track_valid_mask_w.sum(0)

    if huber_delta > 0: # iphone: -1
    ...
    for step in tqdm(range(total_steps)):
        if step == switch_to_ind_step: # NOTE 500
            logging.info(
                "Switch to Independent Camera Optimization and Start Scheduling"
            )
            cams.disable_delta() # NOTE disable delta_flag
        ...
        ########################
        dep_scale = param_scale.abs() # T NOTE all one
        dep_scale = dep_scale / dep_scale.mean() # Normalization

        scaled_depth_list = dep_list * dep_scale[:, None] # MARK: scale depth
        ...
        with torch.no_grad():
            ...
            point_ref_mean = (point_ref * s_track_valid_mask_w[:, :, None]).sum(0) # N,3
            point_ref_std = (point_ref - point_ref_mean[None]).norm(dim=-1, p=2) # T,N
            point_ref_std_robust_w = (point_ref_std * s_track_valid_mask_w).sum(0) # N
            point_ref_std_robust_w = positive_th_gaussian_decay(
                point_ref_std_robust_w, std_decay_th, std_decay_sigma
            ) # N

            robust_w = depth_robust_w * point_ref_std_robust_w[None] # T,N
            cross_robust_time_mask = robust_w[:, None] * robust_w[None, tgt_inds] # Src,Tgt,N
            cross_time_mask = cross_time_mask * cross_robust_time_mask.detach() # Src,Tgt,N

        uv_target = homo_list[None, tgt_inds].expand(
            len(uv_src_to_every_frame), -1, -1, -1
        ) # Src,Tgt,N,2
        uv_loss_i = (uv_src_to_every_frame - uv_target).norm(dim=-1) # Src,Tgt,N
        ...
        warped_depth = point_ref_to_every_frame[..., -1] # Src,Tgt,N

        dep_consistency_i = 0.5 * abs(
            dep_target / torch.clamp(warped_depth, min=1e-6) - 1
        ) + 0.5 * abs(warped_depth / torch.clamp(dep_target, min=1e-6) - 1) # Src,Tgt,N
        ...
        if lambda_cam_smooth_rot > 0 or lambda_cam_smooth_trans > 0: # iphone: 0.0 0.0
            cam_trans_loss, cam_rot_loss = cams.smoothness_loss() # NOTE camera smoothness loss
        else:
        ...
        loss = (
            ...
            + lambda_flow * uv_loss
            + lambda_small_correction * dep_corr_loss # iphone: 0.0, nvidia_rodynrf: 0.03
            + lambda_cam_smooth_rot * cam_rot_loss # iphone: 0.0, nvidia_rodynrf: 0.0
            + lambda_cam_smooth_trans * cam_trans_loss # iphone: 0.0, nvidia_rodynrf: 0.0
        )
        ...
        with torch.no_grad():
            point_ref_mean = (point_ref * s_track_valid_mask_w[:, :, None]).sum(0) # N,3
            std = (point_ref - point_ref_mean[None]).norm(dim=-1, p=2) # T,N
            ...
            if step % viz_verbose_n == 0 or step == total_steps - 1: # viz_verbose = 300
            ...
            viz_flag = viz_flag and step % viz_denser_interval == 0 # viz_denser_interval: 1
            viz_flag = viz_flag or step % viz_fig_n == 0 or step == total_steps - 1 # viz_fig_n: 300
            if viz_flag:
    ...
    make_video_from_pattern(
        ...
        osp.join(log_dir, "static_scaffold_init.gif"),
    )

    if total_steps > 0: # 2000
    ...
    # * save the optimized cameras and the depth scale
    # dep_scale = param_scale.abs()
    dep_scale = param_scale.abs()
    dep_scale = dep_scale / dep_scale.mean()
    logging.info(f"Update the S2D depth scale with {dep_scale}")
    s2d.rescale_depth(dep_scale) # NOTE rescale_depth
    torch.save(cams.state_dict(), osp.join(log_dir, "bundle_cams.pth")) # MARK: save .pth
    torch.save(
        {
            # sol
            "dep_scale": dep_scale,  # ! important, to later rescale the depth
            "dep_correction": param_dep_corr,
            "s_track": s_track, # static track
            "s_track_mask": s_track_valid_mask, # static track mask
        },
        osp.join(log_dir, "bundle.pth"),
    )
    # also save a reconstructed point cloud
    if save_more_flag: # iphone: False
        np.savetxt(
...
def round_int_coordinates(coord, H, W):
    ret = coord.round().long() # N, 2
    valid_mask = (
        (ret[..., 0] >= 0) & (ret[..., 0] < W) & (ret[..., 1] >= 0) & (ret[..., 1] < H)
    )
    ret[..., 0] = torch.clamp(ret[..., 0], 0, W - 1) # strange: why valid mask and clamp
...
def prepare_track_homo_dep_rgb_buffers(s2d, track, track_mask, t_list):
    ...
    ori_dep_list[~track_mask] = -1 # process invalid region
    homo_list[~track_mask] = 0.0   # process invalid region
...
def query_buffers_by_track(image_buffer, track, track_mask, default_value=0.0):
    ...
    T, H, W, C = image_buffer.shape # mask: c == 1; image: c == 3;
    N = track.shape[1]
    ...
    for i in range(T):
        ...
        _value = query_image_buffer_by_pix_int_coord(image_buffer[i].clone(), _int_uv) # N, C
...
def get_world_points(homo_list, dep_list, cams, cam_t_list=None):
    ...
    point_cam = cams.backproject(homo_list.reshape(-1, 2), dep_list.reshape(-1)) # T*M, 3; normalized camera coordinate point
    point_cam = point_cam.reshape(T, M, 3) # T, M, 3
    R_wc, t_wc = cams.Rt_wc_list() # R_wc and t_wc in c2w
    R_wc, t_wc = R_wc[cam_t_list], t_wc[cam_t_list]
    point_world = torch.einsum("tij,tmj->tmi", R_wc, point_cam) + t_wc[:, None] # T, M, 3; normalized world coordinate point
    return point_world
