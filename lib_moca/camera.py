...
class MonocularCameras(nn.Module):
    def __init__(
        ...
        #
        delta_flag=True, # False
        init_camera_pose=None,  # either T_wc in indep model; or T_i(i+1) in delta model
        # * cam flag
        iso_focal=False, # False
    ) -> None:
        super().__init__()

        self.T = n_time_steps
        self.register_buffer("delta_flag", torch.tensor(delta_flag)) # iphone: False, nvidia: Fasle
        self.register_buffer("iso_focal", torch.tensor(iso_focal)) # iphone: False, nvidia: True

        rel_focal, cxcy_ratio = self.__get_init_fc__( # focal -> normalize focal
        ...
        param_cam_q, param_cam_t = self.__get_init_qt__(init_camera_pose) # init_camera_pose: c2w
        self.q_wc = nn.Parameter(param_cam_q) # c2w
        self.t_wc = nn.Parameter(param_cam_t) # c2w

        self.register_buffer("default_H", torch.tensor(default_H)) # s2d.H
        self.register_buffer("default_W", torch.tensor(default_W)) # s2d.W
        self.summary()
        return

    @property
    def rel_focal(self):
        if self.iso_focal: # NOTE isometric focal
    ...
    @classmethod
    def load_from_ckpt(cls, ckpt): # NOTE cls: MonocularCameras
    ...
    def __get_init_fc__(self, fovxfovycxcy, KHW):
        if KHW is None: # iphone: None
            if fovxfovycxcy is None:
                fovxfovycxcy = [53.1, 53.1, 0.5, 0.5]
                logging.warning(
                    f"Both fxfycxcy and KHW are None, use default {fovxfovycxcy}"
                )
            rel_focal_x = 1.0 / np.tan(np.deg2rad(fovxfovycxcy[0]) / 2.0) # strange focal; normalize? NOTE homo_size = 1
            ...
            K, H, W = KHW
            H, W = float(H), float(W)
            L = min(H, W) # normalize the intrinsics, normalize focal
    ...
    def __get_init_qt__(self, init_camera_pose):
        if init_camera_pose is not None:
            if self.delta_flag: # iphone: False
                # T_w0, [T_01, T_12, ...]
                assert len(init_camera_pose) == self.T - 1
                delta_q0 = matrix_to_quaternion(torch.eye(3)[None]) # 1, 3, 3 -> 1, 4
                delta_q = matrix_to_quaternion(init_camera_pose[:, :3, :3]) # T-1, 3, 3, -> T-1, 4
                param_cam_q = torch.cat([delta_q0, delta_q], 0) # T, 4
                delta_t0 = torch.zeros(3)[None] # 1, 3 
                delta_t = init_camera_pose[:, :3, -1] # T-1, 3
                param_cam_t = torch.cat([delta_t0, delta_t], 0) # T, 3
            else:
                # construct independent: T_wc
                init_camera_pose = torch.as_tensor(init_camera_pose) # NOTE init_camera_pose: c2w
                param_cam_q = matrix_to_quaternion(init_camera_pose[:, :3, :3]) # T, 3, 3, -> T, 4 NOTE c2w
                param_cam_t = init_camera_pose[:, :3, -1] # T, 3 NOTE c2w
        else:
    ...
    def T_wc(self, ind): # return c2w
        ...
        if self.delta_flag:
            ...
            R = quaternion_to_matrix(F.normalize(self.q_wc[ind : ind + 1], dim=-1))[0] # self.q_wc: T,4
            t = self.t_wc[ind] # self.t_wc: T,3
            T = torch.eye(4).to(R)
            T[:3, :3] = R
            T[:3, 3] = t
            return T.float()

    def T_cw(self, ind): # return w2c
        T_wc = self.T_wc(ind)
        return torch.linalg.inv(T_wc)

    def Rt_wc(self, ind): # return c2w
        T = self.T_wc(ind)
        return T[:3, :3], T[:3, -1]

    def Rt_cw(self, ind): # return w2c
        R_wc, t_wc = self.Rt_wc(ind)
        R_cw = R_wc.T
        t_cw = -R_cw @ t_wc
        return R_cw, t_cw

    def T_wc_list(self): # return c2w list
        # * core function
        if self.delta_flag:
    ...
    def T_cw_list(self): # return w2c list
        T_wc = self.T_wc_list()
        T_cw = torch.linalg.inv(T_wc)
        return T_cw

    def Rt_wc_list(self): # return c2w list
        T_wc_list = self.T_wc_list() # c2w
        R_wc = T_wc_list[:, :3, :3]
        t_wc = T_wc_list[:, :3, -1]
        return R_wc, t_wc # c2w

    def Rt_cw_list(self): # return w2c list
        R_wc, t_wc = self.Rt_wc_list() # c2w
        R_cw = R_wc.transpose(1, 2) # w2c
        t_cw = -torch.einsum("bij,bj->bi", R_cw, t_wc) # w2c
        return R_cw, t_cw # w2c
    ...
    def smoothness_loss(
        ...
        if t_list is None: # None
            t_list = torch.arange(self.T).to(self.q_wc) # [0, 1, 2, ..., T-1]
        else:
            t_list = torch.as_tensor(t_list).to(self.q_wc)
        t_list = t_list.sort()[0].long() # [0, 1, 2, ..., T-1]
        T_wc_list = self.T_wc_list() # c2w
        T_wc_list = T_wc_list[t_list] # T,4,4
        assert len(shift_list) == len(weight_list)
        loss_rot, loss_trans = 0.0, 0.0
        for delta_t, w in zip(shift_list, weight_list):
            T_a = T_wc_list[:-delta_t] # T-1,4,4 NOTE c2w
            T_b = T_wc_list[delta_t:] # T-1,4,4 NOTE c2w
            T_a_inv = T_a.inverse() # T-1,4,4 w2c
            dT = torch.einsum("tij,tjk->tik", T_a_inv, T_b) # T-1,4,4
            dR = dT[:, :3, :3] # T-1,3,3
            ang_vel = matrix_to_axis_angle(dR).norm(dim=-1) # T-1,3,3 -> T-1,3 -> T-1
            transl_vel = dT[:, :3, -1].norm(dim=-1) # T-1
            if square_flag:
                ang_vel = ang_vel**2
                transl_vel = transl_vel**2
            loss_rot = loss_rot + w * ang_vel.sum()
            loss_trans = loss_trans + w * transl_vel.sum()
        return loss_rot, loss_trans # nvidia marble should very small    
    ################################################################################
    # * Intrinsic
    ################################################################################
    def K(self, H=None, W=None):
        ...
        fx = self.rel_focal[0] * L / 2.0 # ratio -> metric
        fy = self.rel_focal[1] * L / 2.0 # ratio -> metric
        cx = W * self.cxcy_ratio[0] # ratio -> metric
        cy = H * self.cxcy_ratio[1] # ratio -> metric
...
def __project__(xyz, cams, th=1e-5):
    ...
    xy = xyz[..., :2]
    z = xyz[..., 2:]
    z_close_mask = abs(z) < th # hard mask
    if z_close_mask.any():
        # logging.warning(
    ...
    cxcy = torch.as_tensor(cams.cxcy_ratio).to(xyz) * 2.0 - 1.0 # change range: [0, 1] -> [-1, 1], what this?
    uv = (xy * rel_f[None] / z) + cxcy[None, :]
    return uv  # [-1,1] [-H/W, H/W] or [-W/H, W/H] depending on the aspect ratio
...
def __backproject__(uv, d, cams):
    ...
    cxcy = torch.as_tensor(cams.cxcy_ratio).to(uv) * 2.0 - 1.0 # [0, 1] -> [-1, 1]
...
def __get_homo_coordinate_map__(H, W): # u:x, v:y
    # the grid take the short side has (-1,+1)
    if H > W:
        u_range = [-1.0, 1.0]
        v_range = [-float(H) / W, float(H) / W]
