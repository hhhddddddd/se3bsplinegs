...
def rescale_camera_pose(
    ...
    ) # T, N_static_track, 2; T, N_static_track; T, N_static_track, 3; 
    # ! this jitter removal is super important for robustly scale the Nvidia camera back to a correct scale!!
    # NOTE mask out large depth jitter
    neighbor_frame_mask = s_track_mask[1:] * s_track_mask[:-1] # T-1, N_static_track
    depth_diff = abs(dep_list[1:] - dep_list[:-1]) # T-1, N_static_track
    ...
    T, M = homo_list.shape[:2] # T, N_static_track
    point_cam = cams.backproject(homo_list.reshape(-1, 2), dep_list.reshape(-1)) # T*M,3
    point_cam = point_cam.reshape(T, M, 3) # T,M,3
    R_wc, t_wc = cams.Rt_wc_list() # NOTE R_wc (T,3,3) and t_wc (T,3) in c2w
    point_ref_rot = torch.einsum("tij,tmj->tmi", R_wc, point_cam)
    point_ref = point_ref_rot + t_wc[:, None] # world
    # the optimal scale has a closed form solution from a quadratic form

    # we only consider the neighboring two frames for now!
    a = point_ref_rot[1:] - point_ref_rot[:-1] # T-1,M,3
    b = (t_wc[1:] - t_wc[:-1])[:, None].expand(-1, M, -1) # T-1,M,3
    a, b = a[neighbor_frame_mask], b[neighbor_frame_mask] # M_sub,3
    # should be masked
    s_optimal = float(-(a * b).sum() / (b * b).sum())
    # avoid singular case
    if s_optimal < 0.00001: # 0.027156857773661613
        logging.warning(
...
def moca_solve(
    ...
    assert static_id_mode in ["raft", "track"], f"{static_id_mode} not in [raft, track]" # iphone: raft, nvidia: raft
    s2d.to(device)
    H, W, T = s2d.H, s2d.W, s2d.T

    track = s2d.track.cpu() # T, N_track, 2
    track_mask = s2d.track_mask.cpu() # T, N_track
    continuous_pair_list = make_pair_list(T, interval=[1], dense_flag=True) # T-1, 2 [(0,1), (1,2), ..]
    ...
    robust_depth_decay_th = robust_depth_decay_th * depth_median # NOTE 
    ...
    '''
    1. static track 2. process camera 3. optimistic camera 4. visualization
    '''
    # * 1. mark static track
    if static_id_mode == "raft":
        ...
        raft_epi = s2d.epi.clone() # T,H,W
        with torch.no_grad():
            epierr_list = query_buffers_by_track(raft_epi[..., None], track, track_mask) # T, N_all_track, 1 NOTE query dynamic properties
            epierr_list = epierr_list.squeeze(-1).cpu() # T, N_all_track
    ...
    s2d.register_track_indentification(track_static_selection, track_dynamic_selection) # NOTE Save2d
    sta_track = track[:, track_static_selection] # static track 
    sta_track_mask = track_mask[:, track_static_selection] # static track mask

    sta_track_dep = query_buffers_by_track(
        s2d.dep[..., None], sta_track, sta_track_mask
    ).to(sta_track_mask.device) # T, N_static_track, 1 NOTE query depth properties

    assert not torch.isinf(sta_track_dep[sta_track_mask]).any()
    assert not torch.isnan(sta_track_dep[sta_track_mask]).any()

    # verify the static track maximum visible depth and filter out NOTE filer static track
    if depth_filter_th > 0: # iphone: -1; nvidia_rodynrf: 30 
        logging.warning(
        ...
        invalid_mask = sta_track_dep.squeeze(-1) > depth_filter_th # iphone: -1
        ...
        filter_mask = valid_cnt >= depth_filter_min_cnt # iphone: 4
        # filter
    ...
    if gt_cam is None:
        ...
        cams = gt_cam
        if rescale_gt_cam_transl: # iphone: False, nvidia_rodynrf: True
            logging.info(
    ...
    # * 3. optimistic camera bundle adjustment
    if ba_total_steps > 0: # 2000 iphone need compute_static_ba
        compute_static_ba(
    ...
    viz_all_pts = torch.cat(viz_all_pts, 0).numpy() # N_all, 6
    viz_all_sel = np.random.choice(len(viz_all_pts), 50_000, replace=False) # 50_000, 6
    viz_all_pts = viz_all_pts[viz_all_sel] # 50_000, 6
