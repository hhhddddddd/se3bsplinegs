...
def viz_global_ba(
    ...
    mu = point_world.clone()[mask] # N_p,3; position
    sph = RGB2SH(rgb.clone()[mask]) # N_p,3; color
    s = torch.ones_like(mu) * pts_size # N_p,3; scale
    fr = torch.eye(3, device=device).expand(len(mu), -1, -1) # N_p,3,3; rotation
    o = torch.ones(len(mu), 1, device=device) # N_p,1; opacity
    viz_cam_R = quaternion_to_matrix(cams.q_wc) # T,3,3; camera R
    viz_cam_t = cams.t_wc # T,3; camera T
    viz_cam_R, viz_cam_t = cams.Rt_wc_list() # strange
    viz_f = 1.0 / np.tan(np.deg2rad(90.0) / 2.0) # camera focal
    ...
    ) # scene_global_20deg: 480, 480, 3; scene_camera_20deg: 480, 480, 3
    frame = np.concatenate([v for v in frame_dict.values()], 1) # 480, 960, 3
    # * also do color and error if provided
    id_color = cm.hsv(np.arange(M, dtype=np.float32) / M)[:, :3] # M,3
    id_color = torch.from_numpy(id_color).to(device)
    id_color = id_color[None].expand(T, -1, -1) # T,M,3
    sph = RGB2SH(id_color[mask]) # N_p,3; id_color
    ...
    if error is None:
        ...
        frame = np.concatenate([frame, add_frame], 0) # 960, 960, 3
    # imageio.imsave("./debug/viz.jpg", frame)
    frame = frame.copy()
    if len(text) > 0: # test: 'Step=0 ErrorVizTh=2.504594'
