...
torch.autograd.set_detect_anomaly(
    os.environ.get("MOTIONGS_DETECT_ANOMALY", "0") == "1"
)
...
def photometric_warmup(ws, log_path, fit_cfg, nvidia_mode=False):
    ...
    EPI_TH = getattr(fit_cfg, "photo_warm_epi_th", getattr(fit_cfg, "epi_th", 1e-3)) # iphone: 3e-5; nvidia: 0.001; nvidia marble: 1e-5
    PHOTO_STATIC_WARM_STEPS = getattr(fit_cfg, "photo_static_warm_steps", -1) # iphone: -1; nvidia: 6000
    if PHOTO_STATIC_WARM_STEPS < 0: # iphone: -1; nvidia: 6000
    ...
    s2d = (
        ...
        ) # use the output of static_reconstruct to scale depth
        .load_vos()
        .to(device)
    )
    s2d = set_epi_mask_to_s2d_for_bg_render(s2d, EPI_TH, device) # dynamic mask -> static mask
    ...
    photo_solver = DynReconstructionSolver(
        ...
        device=device,
        radius_init_factor=getattr(fit_cfg, "gs_radius_init_factor", 4.0), # 1.0
        opacity_init_factor=getattr(fit_cfg, "gs_opacity_init_factor", 0.95), # 0.95
    )
    ...
    s_model = photo_solver.get_static_model(
        ...
        radius_max=getattr(fit_cfg, "gs_radius_max", 0.1), # 1.0
        max_sph_order=getattr(fit_cfg, "gs_max_sph_order", 0), # 3
    ...
    rgb, dep, alp = photo_solver.render_all(cams, s_model=s_model) # T,C,H,W
    np.savez(
        osp.join(log_path, "photo_warmup_rendered.npz"), # ./data/nvidia/Balloon1/logs/useless_exp_20251017_161517
        rgb=rgb.cpu().numpy(),
        dep=dep.cpu().numpy(),
        alp=alp.cpu().numpy(),
    ) # save render results
    ...
    torch.save(cams.state_dict(), osp.join(log_path, "bundle", "bundle_cams.pth")) # MARK: save .pth

    return


def scaffold_reconstruct(ws, log_path, fit_cfg, nvidia_mode=False):
    seed_everything(SEED)
    DEPTH_DIR, TAP_MODE = auto_get_depth_dir_tap_mode(ws, fit_cfg) # iphone: sensor_depth, bootstapir; nvidia: metric3d_depth, cotracker
    DEPTH_BOUNDARY_TH = getattr(fit_cfg, "depth_boundary_th", 1.0) # iphone: 1.0 NOTE laplacian_filter_depth

    EPI_TH = getattr(fit_cfg, "epi_th", 1e-3) # iphone: 3e-5; nvidia: 0.001; nvidia marble: 1e-5
    DYN_ID_CNT = getattr(fit_cfg, "dyn_id_cnt", 2 * 4) # iphone: 8; nvidia: 2; nvidia marble: 4
    SCF_GEO_KEYFRAME_RATE = getattr(fit_cfg, "scf_geo_keyframe_rate", 4) # iphone: 4; nvidia: 1; nvidia marble: 2
    DEP_MEDIAN = getattr(fit_cfg, "dep_median", 1.0) # iphone: -1; nvidia: -1
    device = torch.device("cuda:0")
    ...
    '''
    1. refine dynamic mask 2. get dynamic curves 3. get scaffold 4. optimize scaffold 5. expand scaffold
    '''

    # * 1. re-identify the static and dynamic regions consider static render error mask
    consider_photo_error_dyn_id_th = getattr(
        fit_cfg, "consider_photo_error_dyn_id_th", -1
    ) # iphone: -1
    if consider_photo_error_dyn_id_th > 0: # iphone: -1, nvidia_rodynrf: -1
    ...
    s2d = update_s2d_track_identification(
        ...
        photo_error_masks=photo_error_masks, # None
    ...
    # * 2. get and filter dynamic curves
    sub_t_list = [
        t for t in range(s2d.T) if t % SCF_GEO_KEYFRAME_RATE == 0 or t == s2d.T - 1
    ] # iphone: [0, 4, 8, 12, ..., 468, 472, 474]; nvidia: [0, 1, 2 .., 9, 10, 11]
    ...
    ) # curve_xyz:[T_sub,N_dyn_filter,3], curve_mask:[T_sub,N_dyn_filter], curve_rgb:[T_sub,N_dyn_filter,3], curve_filter_mask:[N_dyn],
    curve_uv = s2d.track[:, s2d.dynamic_track_mask][:, curve_filter_mask][sub_t_list][
        ..., :2
    ] # [T_sub,N_dyn_filter,2]
    ...
    if refilter_curve_by_photo_error_cnt > 0: # iphone: -1; nvidia: -1
        photo_error_masks = get_static_render_error_mask(
            s2d,
            log_path,
    ...
    viz_mosca_curves_before_optim(curve_xyz, curve_rgb, curve_mask, cams, log_path) # vis
    ...
    scaffold: MoSca = MoSca(
        ...
        skinning_method=getattr(fit_cfg, "mosca_skinning_method", "dqb"), # dqb or lbs
        # skinning_method="lbs", # NOTE dqb or lbs, spin uses lbs will cuda mem error 
    ...
    scaffold.compute_rotation_from_xyz() # compute rotation again for setting self._node_rotation.data
    if getattr(fit_cfg, "mosca_resample_flag", True): # True
        sampled_inds = scaffold.resample_node(resample_factor=1.0, use_mask=True) # NOTE downsample nodes Important!!!
    else:
        logging.warning("Not resampling the scaffold")
        sampled_inds = torch.arange(scaffold.M).to(device)
    node_rgb = curve_rgb[sampled_inds] # [N_dyn_filter_downsample,3]
    ...
    # * 4. Optimize the curve with ARAP NOTE As-Rigid-As-Possible
    assert (
        getattr(fit_cfg, "geo_mosca_use_mask_topo", True) or s2d.track.shape[-1] == 3
    ), "Must use mask topo for 2D tracks"
    if getattr(fit_cfg, "geo_mosca_steps", 1500) > 0: # 1500
        scaffold = geometry_scf_init(
            ...
            viz_interval=getattr(fit_cfg, "geo_mosca_viz_interval", -1), # strange
    ...
    # * 5. expand scaffold NOTE subtime -> alltime
    if SCF_GEO_KEYFRAME_RATE > 1: # 4
        fulltime_curve_mask = s2d.track_mask.detach().clone()[
            :, s2d.dynamic_track_mask
        ][:, curve_filter_mask][:, sampled_inds] # T_all,N_dyn_filter_downsample
...
def photometric_reconstruct(ws, log_path, fit_cfg, nvidia_mode=False):
    seed_everything(SEED)
    DEPTH_DIR, TAP_MODE = auto_get_depth_dir_tap_mode(ws, fit_cfg) # iphone: sensor_depth, bootstapir; nvidia: metric3d_depth, cotracker
    DEPTH_BOUNDARY_TH = getattr(fit_cfg, "depth_boundary_th", 1.0) # iphone: 1.0; nvidia: 0.3 NOTE laplacian_filter_depth
    DEP_MEDIAN = getattr(fit_cfg, "dep_median", 1.0) # iphone: -1; nvidia: -1

    EPI_TH = getattr(fit_cfg, "epi_th", 1e-3) # iphone: 3e-5; nvidia: 0.001
    DYN_ID_CNT = getattr(fit_cfg, "dyn_id_cnt", 2 * 4) # iphone: 8; nvidia: 2

    STATIC_GS_START_OPA = getattr(fit_cfg, "gs_static_start_opacity", 0.01) # iphone: 0.01; nvidia: -1
    DYNAMIC_GS_START_OPA = getattr(fit_cfg, "gs_dynamic_start_opacity", 0.02) # iphone: 0.02; nvidia: 0.009
    ...
    s2d = (
        ...
        .load_dep(DEPTH_DIR, DEPTH_BOUNDARY_TH, nvidia_mode)
        .normalize_depth(median_depth=DEP_MEDIAN)
        .recompute_dep_mask(depth_boundary_th=DEPTH_BOUNDARY_TH) # after normalize depth
        .load_track(
            TAP_MODE, min_valid_cnt=getattr(fit_cfg, "tap_loading_min_valid_cnt", 4)
        )
        .rescale_perframe_depth_from_bundle(
        ...
        .load_vos() # video object segmentation
        .load_flow() # iphone: flow_raft
        .to(device)
    )
    s2d = set_epi_mask_to_s2d_for_bg_render(s2d, EPI_TH, device) 
    s2d.load_zero123() # load zero123

    track_identification = np.load(osp.join(log_path, "track_identification.npz")) # track dynamic static segmentation
    ...
    # reset the scaffold mlevel config NOTE ARAP Loss
    scaffold.set_multi_level(
        mlevel_arap_flag=True,
        mlevel_list=getattr(fit_cfg, "photo_mlevel_list", [1, 6]), # [1, 6]
        mlevel_k_list=getattr(fit_cfg, "photo_mlevel_k_list", [16, 8]), # [16, 8]
        mlevel_w_list=getattr(fit_cfg, "photo_mlevel_w_list", [0.4, 0.3]), # [0.4, 0.3]
    )

    # NOTE construct se(3) b-spline 
    scaffold.init_control_state()
    # start_t=time.time()
    scaffold.compute_se3_bspline()
    # print(f"compute se3 bspline {time.time()-start_t:.2f}s")
    
    '''
    1. construct GS models 2. photometric optimization
    '''

    # * 1. construct GS models
    photo_solver = DynReconstructionSolver(
        working_dir=log_path,
        device=device,
        radius_init_factor=getattr(fit_cfg, "gs_radius_init_factor", 4.0), # NOTE scale factor
        opacity_init_factor=getattr(fit_cfg, "gs_opacity_init_factor", 0.95), # NOTE opacity factor
    )
    # compute fg mask! warning, this mask is only useful for constructing the model.
    photo_solver.identify_fg_mask_by_nearest_curve(
        s2d, cams, "gs_model_construct_fg_mask.gif"
    ) # NOTE 2D image dynamic static segmentation
    if GS_BACKEND == "gof": # iphone: native_add3; nvidia: native_add3
        photo_solver.compute_normals_for_s2d(
            s2d,
            cams,
            patch_size=7,
            nn_dist_th=0.03,
            nn_min_cnt=4,
            viz_fn="gs_model_construct_normal.gif",
        )
    include_fg_in_static = getattr(fit_cfg, "gs_include_fg_in_static", True) # iphone: True, nvidia: True
    s_model_warmup_path = osp.join(
        log_path, f"static_warm_s_model_{GS_BACKEND.lower()}.pth"
    )
    if osp.exists(s_model_warmup_path): # load warmup ckpt, iphone: False, nvidia: True
        ...
        s_model = photo_solver.get_static_model(
            s2d=s2d,
            cams=cams,
            n_init=getattr(fit_cfg, "gs_static_n_init", 30000), # 30000
            radius_max=getattr(fit_cfg, "gs_radius_max", 0.1), # 0.03
            max_sph_order=getattr(fit_cfg, "gs_max_sph_order", 0), # 3
            mask_type="depth" if include_fg_in_static else "static_depth", # depth
        )

    if getattr(fit_cfg, "photo_d_model_fetch_only_error_th", -1) > 0: # iphone: -1; nvidia_rodynrf: 0.1
        photo_data = np.load(osp.join(log_path, "photo_warmup_rendered.npz"))
        render_error_th = getattr(fit_cfg, "photo_d_model_fetch_only_error_th", -1) # 0.1
        rgb_rendered = torch.tensor(photo_data["rgb"]).to(device).permute(0, 2, 3, 1) # T,H,W,3
        render_error = abs(s2d.rgb - rgb_rendered).max(dim=-1).values
        render_error_mask = render_error > render_error_th # T,H,W
        render_error_mask_viz = (render_error_mask[..., None] * s2d.rgb).cpu().numpy()
        imageio.mimsave(
            osp.join(
                log_path,
        ...
        d_model_add_mask = render_error_mask # NOTE 'dynamic' mask <=> static error region
    else:
    ...
    d_model = photo_solver.get_dynamic_model(
        ...
        cams=cams,
        scf=scaffold,
        n_init=getattr(fit_cfg, "gs_dynamic_n_init", 30000), # 30000
        radius_max=getattr(fit_cfg, "gs_radius_max", 0.1), # 0.03
        max_sph_order=getattr(fit_cfg, "gs_max_sph_order", 0), # 3
        leaf_local_flag=getattr(fit_cfg, "gs_leaf_local_flag", True), # True
        additional_mask=d_model_add_mask, # iphone: None; nvidia: render_error_mask
        nn_fusion=getattr(fit_cfg, "gs_nn_fusion", -1), # iphone: -1
        # ! below is set to dyn_gs_model becaues it controls the densification
        max_node_num=getattr(fit_cfg, "gs_max_node_num", 100000), # 100000
    )
    with torch.no_grad():
        if DYNAMIC_GS_START_OPA > 0: # opacity, iphone: 0.02; nvidia: 0.009; nvidia_marbles: 0.02
        ...
        if STATIC_GS_START_OPA > 0: # opacity, iphone: 0.01; nvidia: -1; nvidia_marbles: 0.01
    ...
    photo_solver.photometric_fit(
        ...
        lambda_zero123_sds=getattr(fit_cfg, "photo_lambda_zero123_sds", 0.001),
        use_zero123_sds_loss=getattr(fit_cfg, "photo_use_zero123_sds_loss", True),
        zero123_sds_start_step=getattr(fit_cfg, "photo_zero123_sds_start_step", 100),
        zero123_sds_interval=getattr(fit_cfg, "photo_zero123_sds_interval", 4),
        zero123_sds_render_reso_min=getattr(fit_cfg, "photo_zero123_sds_render_reso_min", 128),
        zero123_sds_render_reso_mid=getattr(fit_cfg, "photo_zero123_sds_render_reso_mid", 192),
        zero123_sds_render_reso_max=getattr(fit_cfg, "photo_zero123_sds_render_reso_max", 256),
    )
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nvidia_mode", action="store_true", help="open nvidia mode")
    parser.add_argument("--exp_name", type=str, help="experiment name", default="useless_exp")
    parser.add_argument("--logdir", type=str, help="load ckpt")
    ...
    logdir = setup_recon_ws(args.ws, fit_cfg=cfg, exp_name=args.exp_name) # backup code & save config
    # logdir = args.logdir # NOTE debug

    # * RUN
    static_reconstruct(args.ws, logdir, cfg, args.nvidia_mode) # 2000
    photometric_warmup(args.ws, logdir, cfg, args.nvidia_mode) # 6000 
    scaffold_reconstruct(args.ws, logdir, cfg, args.nvidia_mode) # 4000
    photometric_reconstruct(args.ws, logdir, cfg, args.nvidia_mode) # 8000 
    ...
    test_main(
        ...
        tto_flag=True, # NOTE tto_test
        eval_also_dyncheck_non_masked=False,
        skip_test_gen=False,
        nvidia_mode=args.nvidia_mode, # False
        optimizer_tion=True, # True
        eval_background_covisible_mask=True, # False
    )
    test_fps(saved_dir=logdir)
