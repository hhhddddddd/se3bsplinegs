...
class DynSCFGaussian(nn.Module):
    ...
    def __init__(
        ...
        self.min_num_gs = min_num_gs # 32

        self.register_buffer("nn_fusion", torch.tensor(nn_fusion)) # -1
        logging.info(f"ED Model use default {nn_fusion} nearest frame fusion")
        self.register_buffer("dyn_o_flag", torch.tensor(dyn_o_flag).bool()) # True
        if dyn_o_flag: # True
            logging.warning(
        ...
        self.node_ctrl = node_ctrl # True
        if self.node_ctrl: # True
            logging.info(
                f"Node control is enabled! (max number of nodes {max_node_num})"
            )
        # self.max_node_num = max_node_num
        self.register_buffer("max_node_num", torch.tensor(max_node_num)) # 100000

        self.register_buffer("w_correction_flag", torch.tensor([0]).squeeze().bool()) # False
        self.register_buffer(
            "leaf_local_flag", torch.tensor([leaf_local_flag]).squeeze().bool() # True, NOTE local coordinate system
        )

        # * prepare activation
        self.register_buffer("max_scale", torch.tensor(max_scale).to(self.device)) # 0.03
        self.register_buffer("min_scale", torch.tensor(min_scale).to(self.device)) # 0.0
        self.register_buffer(
        ...
        sph_rest_dim = 3 * (sph_order2nfeat(self.max_sph_order) - 1) # 45 = 3 * (16 - 1)
        self._features_dc = nn.Parameter(torch.zeros(init_N, 3)) # N,3
        self._features_rest = nn.Parameter(torch.zeros(init_N, sph_rest_dim)) # N,45
        self._skinning_weight = nn.Parameter(torch.zeros(init_N, self.scf.skinning_k)) # N,16
        self._dynamic_logit = nn.Parameter(self.o_inv_act(torch.ones(init_N, 1) * 0.99)) # NOTE dynamic degree, only for dynamic gaussian
        # * leaf important status
        self.register_buffer("attach_ind", torch.zeros(init_N).long()) # N, NOTE the index of mosca node which is the nearest node for gs in tid.
        ...
        self.return_cate_colors_flag = False # NOTE do not return cate color
    ...
    def get_cate_color(self, color_plate=None, perm=None):
        if not hasattr(self, "group_colors"):
            ...
            self.group_colors = color_plate # [1,3]
            self.group_sphs = RGB2SH(self.group_colors) # [1,3]
        if perm is None:
        ...
        for ind in perm:
            ...
            cate_sph[gs_group_id == gid] = self.group_sphs[ind].unsqueeze(0) # [N,3]
            index_color_map[gid] = self.group_colors[ind] # [1,3]
    ...
    def get_xyz(self):
        if self.leaf_local_flag: # True
            # NOTE USE NEW NODE
            nn_ref_node_xyz, sk_ref_node_quat = self.scf.get_async_knns(
                self.ref_time, self.attach_ind[:, None]
            ) # N_dyn,1,3; N_dyn,1,4
            nn_ref_node_xyz = nn_ref_node_xyz.squeeze(1) # N_dyn,3; NOTE c2w
            nn_ref_node_R_wi = q2R(sk_ref_node_quat.squeeze(1)) # N_dyn,3,3; NOTE c2w
            return (
                torch.einsum("nij,nj->ni", nn_ref_node_R_wi, self._xyz)
                + nn_ref_node_xyz
            ) # global world coordinate system -> local node coordinate system
        else:
            return self._xyz
    ...
    def get_R_mtx(self):
        if self.leaf_local_flag: # True
            ...
            nn_ref_node_R_wi = q2R(sk_ref_node_quat.squeeze(1)) # N_dyn,3,3; NOTE c2w
            return torch.einsum("nij,njk->nik", nn_ref_node_R_wi, q2R(self._rotation)) # global world coordinate system -> local node coordinate system
        else:
            return q2R(self._rotation)

    def get_xyz_se3_bspline(self):
        if self.leaf_local_flag: # True
            # NOTE USE NEW NODE !!! NEED VAST TIME !!!
            nn_ref_node_xyz, sk_ref_node_quat = self.scf.get_async_knns_se3_bspline(
                self.ref_time, self.attach_ind[:, None]
            ) # N_dyn,1,3; N_dyn,1,4
            nn_ref_node_xyz = nn_ref_node_xyz.squeeze(1) # N_dyn,3; NOTE c2w
            nn_ref_node_R_wi = q2R(sk_ref_node_quat.squeeze(1)) # N_dyn,3,3; NOTE c2w
    ...
    def get_R_mtx_se3_bspline(self):
        if self.leaf_local_flag: # True
            # NOTE USE NEW NODE
            _, sk_ref_node_quat = self.scf.get_async_knns_se3_bspline(
                self.ref_time, self.attach_ind[:, None]
            ) # N_dyn,1,4
            nn_ref_node_R_wi = q2R(sk_ref_node_quat.squeeze(1)) # N_dyn,3,3; NOTE c2w
            return torch.einsum("nij,njk->nik", nn_ref_node_R_wi, q2R(self._rotation)) # global world coordinate system -> local node coordinate system
    ...
    def set_surface_deform(self):
        ...
        self.w_correction_flag = torch.tensor(True).to(self.device) # True
        self.scf.fixed_topology_flag = torch.tensor(True).to(self.device) # True
        return
    ...
    def set_inference_mode(self):
        ...
        self.fast_inference_flag = True # NOTE open fast inference flag
        logging.warning((f"ED Model convert to inference mode"))
        return

    def forward(self, t: int, active_sph_order=None, nn_fusion=None, soft_segment=True, scale=5.0):
        assert t < self.T, "t is out of range!"
        if active_sph_order is None:
            active_sph_order = int(self.max_sph_order) # 3
        else:
            assert active_sph_order <= self.max_sph_order # 3

        s = self.get_s # N_dyn,3
        o = self.get_o # N_dyn,1
        sph_dim = 3 * sph_order2nfeat(active_sph_order) # 48
        sph = self.get_c[:, :sph_dim] # N_dyn,48

        if self.fast_inference_flag: # False NOTE pre-baked data
            ...
            mu_live, fr_live = self.scf.warp(
                attach_node_ind=self.attach_ind, # N_dyn, NOTE the index of mosca node which is the nearest node for gs in ref time.
                # query_xyz=self.get_xyz(), # N_dyn,3 NOTE leaf_local_flag
                # query_dir=self.get_R_mtx(), # N_dyn,3,3 NOTE leaf_local_flag
                query_xyz=self.get_xyz_se3_bspline(), # N_dyn,3 NOTE process dynamic gaussian xyz according to ref_node
                query_dir=self.get_R_mtx_se3_bspline(), # N_dyn,3,3 NOTE process dynamic gaussian xyz according to ref_node
                query_tid=self.ref_time, # N_dyn
                target_tid=t,
                skinning_w_corr=(
                    self._skinning_weight if self.w_correction_flag else None
                ), # None -> N_dyn,16
                dyn_o_flag=self.dyn_o_flag, # True
            ) # [N_dyn,3] [N_dyn,3,3]
            # print(f"warp in {time.time()-start_t:.2f}s") 

        if nn_fusion is None: # None
            nn_fusion = self.nn_fusion.item() # -1
        if nn_fusion > 0: # -1
            # filter the gaussian by nn mask
            with torch.no_grad():
                supporting_t = torch.unique(self.ref_time) # T_all
                # find nearest supporting t
                t_dist = (supporting_t - t).abs()
                supporting_t = supporting_t[torch.argsort(t_dist)[:nn_fusion]] # T_nn
                # get the nn mask
                filter_mask = (self.ref_time[:, None] == supporting_t[None]).any(dim=1) # N_dyn TODO hard -> soft
            ...
            assert (
                not self.return_cate_colors_flag  # False
            ), " not support cate color in nn fusion"
        if soft_segment: # True NOTE soft segmentation strategy
            weight = torch.sigmoid(scale * (1 - (torch.abs(self.T - self.ref_time)/self.T))) # N_dyn
            o = weight[..., None] * o

        if self.return_cate_colors_flag: # False
            # logging.warning(f"VIZ purpose, return the cate-color")
            cate_sph, _ = self.get_cate_color()
            sph = torch.zeros_like(sph) # N_dyn,48
    ...
    def append_new_gs(self, optimizer, tid, mu_w, quat_w, scales, opacity, rgb):
        ...
        _, attach_ind, _ = knn_points(mu_w[None], self.scf._node_xyz[tid][None], K=1) # 1,N_mu,K; gs's nearest mosca node in tid 
        self.attach_ind = torch.cat([self.attach_ind, attach_ind[0, :, 0]], dim=0) # N_mu
        ...
        if self.leaf_local_flag: # True
            attach_node_xyz = self.scf._node_xyz[tid][attach_ind[0, :, 0]] # N_mu,3
            attach_node_R_wi = q2R(self.scf._node_rotation[tid][attach_ind[0, :, 0]]) # N_mu,3,3 NOTE c2w
            mu_save = torch.einsum(
                "nji,nj->ni", attach_node_R_wi, mu_w - attach_node_xyz
            ) # N_mu,3
            R_new = torch.einsum("nji,njk->nik", attach_node_R_wi, q2R(quat_w)) # N_mu,3,3
            quat_save = matrix_to_quaternion(R_new) # N_mu,4
        else:
        ...
        self._densification_postprocess(
            ...
            new_skinning_w=torch.zeros(len(mu_w), self.scf.skinning_k).to(mu_w), # all zeros
    ...
    def append_new_node_and_gs(
        ...
        # ! all uncovered
        new_node_xyz_l = subsample_vtx(mu_w, self.scf.spatial_unit) # N_sub,3 NOTE Downsample for node
        new_node_quat_l = (
        ...
        succ = self.scf.append_nodes_pnt(
            ...
            new_node_quat_l,
            tid,
        ) # add node only from pose in one time
        if not succ:
        ...
        if self.leaf_local_flag:
            ...
            mu_save, quat_save = mu_w, quat_w # [N_all,3] [N_all,4]

        # finally update the parameters
        old_N = self.N # number dynamic gaussian
        self._densification_postprocess(
            optimizer,
            new_xyz=mu_save,  # ! store the position in live frame
            new_r=quat_save,
    ...
    @torch.no_grad()
    def prune_control_points(self):
        # TODO
        self.scf.control_state = self.scf.control_state
    ...
    def prune_nodes(self, optimizer, prune_sk_th=0.02, viz_fn=None):
        ...
        acc_w = self.get_node_sinning_w_acc("max") # M
        # viz
        ...
        node_prune_mask = prune_mask_sk & prune_mask_carry # M NOTE NODE PRUNE MASK
        logging.info(
        ...
        if leaf_pruning_mask.any():
            self._prune_points(optimizer, leaf_pruning_mask) # prune dynamic gaussian
    ...
    def gradient_based_node_densification(
        ...
        # * 1. add node * 2. add dynamic gaussian
        grad = self.corr_gradient_accum / (self.corr_gradient_denom + 1e-6) # N_dyn: dynamic gaussian number
        # grad += 0.75
        candidate_mask = grad > gradient_th  # N_dyn
        ...
        gs_mu_list = torch.stack(gs_mu_list, dim=0)  # T,N_dyn_densify,3
        gs_fr_list = torch.stack(gs_fr_list, dim=0)  # T,N_dyn_densify,3,3
        ...
        new_node_xyz = gs_mu_list[:, resample_ind] # T,new_M,3
        new_node_quat = matrix_to_quaternion(gs_fr_list[:, resample_ind]) # T,new_M,4

        # append these new node into scf
        old_M = self.scf.M
        self.scf.append_nodes_traj(
            optimizer,
            new_node_xyz,  # T,new_M,3
            new_node_quat, # T,new_M,4
        ...
        original_attach_ind = self.attach_ind[candidate_mask][resample_ind] # new_M NOTE attach index is ref node index

        new_attach_ind_list, new_gs_ind_list = [], []
        for _i in range(new_node_xyz.shape[1]): # new_M
            _attach_ind = old_M + _i
            # the same carrying leaves duplicate them
            neighbors_mask = self.attach_ind == original_attach_ind[_i] # N_dyn
            ...
            #
            gs_ind = torch.arange(self.N, device=self.device)[neighbors_mask] # N_sub
        ...
        self.clean_corr_control_record() # corr_gradient_accum and corr_gradient_denom
        return
...
def subsample_vtx(vtx, voxel_size): # Voxel Grid Subsampling
    # vtx: N,3
