...
def Rt2dq(R, t): # RT -> Dual Quaternion
    ...
    p = matrix_to_quaternion(R.clone()) # N,K,4
    t = torch.cat([torch.zeros_like(t[..., :1]), t.clone()], dim=-1) # N,K,4
    q = 0.5 * quaternion_multiply(t, p)
    dq = torch.cat([p, q], dim=-1) # N,K,8
    # with torch.no_grad():
...
def dq2Rt(dq): # Dual Quaternion -> RT
