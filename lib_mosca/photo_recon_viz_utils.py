...
def __get_move_around_cam_T_cw__(
    ...
    if center_dep < 1e-2:
        try:
            ...
            center_dep = 1.0
    focus_point = torch.Tensor([0.0, 0.0, center_dep]).to(depth) # NOTE focus points
    ...
    for i in range(total_steps):
        ...
        T_new_w = T_w_new.inverse() # 4,4
        move_around_view_list.append(T_new_w)
    return move_around_view_list # T,4,4
...
def viz2d_total_video(
    ...
    frame_list = []
    fix_cam_id = start_from # 0

    # prepare the novel camera poses
    move_around_angle = np.deg2rad(move_around_angle_deg)
    if move_around_id is None:
        move_around_id = (start_from + end_at) // 2 # middle time
    move_around_view_list = __get_move_around_cam_T_cw__( 
        s_model,
        d_model,
        cams,
        move_around_id, # middle time
        move_around_angle, # pi / 18 NOTE angle degree
        start_from, # 0
        end_at, # T-1
    ) # T,4,4 NOTE generate pseudo camera pose
    for _ind, view_ind in tqdm(enumerate(range(start_from, min(end_at + 1, cams.T)))):
        ...
        frame = viz2d_one_frame(
            ...
            d_model,
            subsample=1,
            loss_mask_type=mask_type, # all
            print_text=print_text, # False
        ) # H*4,W*3,3
        # fix cam, vary time
        _frame = viz2d_one_frame(
            ...
            append_dyn=False,
            append_graph=False,
            prefix_text=f"Fix={fix_cam_id} ", # NOTE fix camera: 0
            T_cw=cams.T_cw(fix_cam_id), # fix camera pose
            print_text=print_text,
        ) # H,W*3,3
        frame = np.concatenate([frame, _frame], 0) # H*5,W*3,3
        # fix time, vary cam
        _frame = viz2d_one_frame(
            move_around_id, # middle time
            s2d,
            ...
            append_dyn=False,
            append_graph=False,
            prefix_text=f"T={move_around_id} ", # NOTE fix time: middle time
            T_cw=move_around_view_list[_ind], # camera pose
            print_text=print_text,
        )  # H,W*3,3
        frame = np.concatenate([frame, _frame], 0) # H*6,W*3,3
        # vary cam and time
        _frame = viz2d_one_frame(
            view_ind,
            s2d,
        ...
        frame = np.concatenate([frame, _frame], 0) # H*7,W*3,3

        frame_list.append(frame)

    # viz the flow
    if d_model is not None:
        flow_frame_list1, choice = viz2d_flow_video( # NOTE "WithBG"
            start_from,
            end_at,
            cams,
            s_model,
        ...
        flow_frame_list2, choice = viz2d_flow_video( # NOTE "NoBG"
            start_from,
            end_at,
            cams,
            s_model,
        ...
        flow_frame_list3, choice = viz2d_flow_video( # NOTE "FixCam"
            start_from,
            end_at,
            cams,
            s_model,
    ...
    for frame in final_frame_list:
        H = frame.shape[0] # H*8
        frame_top = frame[: H // 2] # H*4,W*3,3
        frame_bottom = frame[H // 2 :] # H*4,W*3,3
        if remove_redundant_flag: # True
        ...
        frame = np.concatenate([frame_top, frame_bottom], 1) # H*4,W*6,3
        re_aranged_list.append(frame) # T,H*4,W*6,3
    ...
    while cur < T:
        ...
        cnt += 1
        cur += max_num_frames # max_num_frames: 500 NOTE strange
...
def viz2d_one_frame(
    ...
    gs5 = [s_model()]
    if d_model is not None:
        gs5.append(d_model(model_tid)) # time
    ...
    if append_graph and d_model is not None:
        viz_frame_graph = make_viz_graph(d_model, model_tid, cams, view_cam_id) # H,W*3,3
        viz_frame = np.concatenate([viz_frame, viz_frame_graph], 0) # H*2,W*3,3
...
def make_viz_graph(d_model, view_ind, cams, view_cam_id=None, max_radius=0.001):
    ...
    c_id = torch.from_numpy(cm.hsv(order / len(node_mu))[:, :3]).to(node_mu) # M,3 NOTE node color by id

    c_time = torch.from_numpy(cm.hsv(torch.rand(len(node_mu)))[:, :3]).to(node_mu) # M,3 NOTE node color by time
    acc_w = d_model.get_node_sinning_w_acc().detach().cpu().numpy() # M
    acc_w_binary = (acc_w > float(d_model.scf.skinning_k) / 2.0).astype(np.float32) # M
    acc_w = acc_w / acc_w.max() # M
    c_w = torch.from_numpy(cm.viridis(acc_w)[:, :3]).to(node_mu) # M,3 NOTE node color by acc_w
    c_wb = torch.from_numpy(cm.viridis(acc_w_binary)[:, :3]).to(node_mu) # M,3
    ...
    viz_r = min(max_radius, d_model.scf.spatial_unit / 10.0) # 0.001 NOTE for scale
    for color, text in zip(
        [c_id, c_time, c_w], ["Nodes-id", "Nodes-rand-color", "Nodes-acc-w"]
    ):
        sph = RGB2SH(color)
        fr = torch.eye(3).to(node_mu)[None].expand(len(node_mu), -1, -1) # M,3,3
        s = torch.ones(len(node_mu), 3).to(node_mu) * viz_r # M,3
        o = torch.ones(len(node_mu), 1).to(node_mu) * 1.0 # M,1
...
def viz3d_total_video(
    ...
    frames_node = viz_curve(
        ...
        torch.ones_like(d_model.scf._node_xyz[..., 0]).bool(), # T,M
        cams,
        res=res, # 480
        text="All ED Nodes",
        viz_n=-1,
    ...
    while cur < T:
        __video_save__(
            save_path[:-4] + f"_{cnt}.gif", frames[cur : cur + max_num_frames]
        )
        cnt += 1
        cur += max_num_frames # max_num_frames: 500
    return
...
def viz3d_scene_video(
    ...
    if save_dir is not None:
        for k, v in frames.items():
            __video_save__(
                osp.join(save_dir, f"{prefix}dyn_{k}_{start_tid}-{end_tid}.gif"),
                v,
            )
    return frames
...
def __video_save__(fn, imgs, fps=10):
    ...
    # out.release()
    try: 
        imageio.mimsave(fn, imgs) # when photometric_3Dviz.gif Fail!
        logging.info(f"Saved!")
    except:
        imgs_uint8 = [(img).astype(np.uint8) for img in imgs]
        imageio.mimsave(fn, imgs_uint8) # when photometric_3Dviz.gif Fail!
        logging.info(f"Saved!")
    return
