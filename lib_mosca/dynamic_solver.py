...
@torch.no_grad()
def line_segment_init(track_mask, point_ref): # Input: T,N; T,N,3 Output: T,N,3
    ...
    tracl_mask_valid_cnt = track_mask.sum(0) # N
    working_mask = tracl_mask_valid_cnt > 0 # N
    ...
    inverse_muti = torch.Tensor([i + 1 for i in range(T)][::-1]).to(working_point_ref) # [T, T-1, T-2, ..., 3, 2, 1]
    for t in tqdm(range(T)):
        to_fill_mask = ~working_track_mask[t] # to_fill_mask.sum() == N_fill
        ...
        if t == T - 1:  # if right end, use the previous one
            value = working_point_ref[t - 1, to_fill_mask].clone() # N_fill,3
        else:
    ...
    ret = point_ref.clone() # T,N,3
    ret[:, working_mask] = working_point_ref # T,N,3
...
def get_dynamic_curves(
    ...
    # * load track
    if return_all_curves: # mosca: False, construction: True
        track = s2d.track.clone()
        track_mask = s2d.track_mask.clone()
    else:
        track = s2d.track[:, s2d.dynamic_track_mask].clone() # T,N_dyn,2
        track_mask = s2d.track_mask[:, s2d.dynamic_track_mask].clone() # T,N_dyn
    if fg_additional_mask is not None: # None
    ...
    if t_list is None:
        ...
        t_list = torch.as_tensor(t_list).to(device) # use sub_list

    track = track[t_list] # T_sub,N_dyn,2
    track_mask = track_mask[t_list] # T_sub,N_dyn
    ...
    if track.shape[-1] == 3:
        ...
        if refilter_2d_track_flag and not return_all_curves: # True and not False
            inlier_mask = slot_o3d_outlier_identifyication(
                curve_xyz,
                track_mask,
                nb_neighbors=refilter_o3d_nb_neighbors,
            ...
            track_mask = track_mask * inlier_mask # NOTE update track_mask

            inlier_mask = curve_shaking_identification(curve_xyz, refilter_shaking_th) # T,N; True: don't shaking
            logging.info(
                f"shaking outlier ratio {(~inlier_mask).float().mean()*100.0:.2f}%"
            )
            if refilter_remove_shaking_curve: # True
                has_shaking = track_mask * (~inlier_mask)  # valid but has outlier
                has_shaking = has_shaking.any(0, keepdim=True)
                logging.info(f"{has_shaking.sum()} curves has shaking, remove")
                inlier_mask = (~has_shaking).expand(len(track_mask), -1)
            ...
            if not refilter_2d_track_only_mask: # not False
                # find valid cnt
                valid_cnt = track_mask.sum(0) # N_dyn
                if filter_mask.sum() > min_num_curves:
                    filter_mask = (valid_cnt >= refilter_min_valid_cnt) & filter_mask
                    logging.info(
                        f"Refiltering 2D tracks, {(~filter_mask).sum()} curves has less than {refilter_min_valid_cnt} valid slots, remove"
                    )
                track = track[:, filter_mask] #  # T_sub,N_dyn_filter,2 NOTE filter dynamic track
                track_mask = track_mask[:, filter_mask]  #  # T_sub,N_dyn_filter NOTE filter dynamic track
                # * NOTE redo
                homo_list, dep_list, rgb_list = prepare_track_buffers(
                    s2d, track[..., :2], track_mask, t_list
                ) # T_sub,N_dyn_filter,C
                curve_xyz = line_segment_init(
                    track_mask,
                    get_world_points(homo_list, dep_list, cams, t_list).clone(),
                ) # T_sub,N_dyn_filter,3
            else:
...
def __compute_physical_losses__(
    scf,
    temporal_diff_shift: list, # [2 8 16]
    temporal_diff_weight: list, # [0.6 0.4 0.3]
    max_time_window: int, # T_all + 1
    reduce="sum",
    square=False, # False
):
    ...
    loss_coord, loss_len = scf.compute_arap_loss(
        tids=sup_tids, # [0, 1, 2, ...]
        temporal_diff_shift=temporal_diff_shift, # [2 8 16]
        temporal_diff_weight=temporal_diff_weight, # [0.6 0.4 0.3]
        reduce_type=reduce, # 'sum'
        square=square, # False
    ) # torch.size([])
...
def geometry_scf_init(
    ...
    os.makedirs(viz_dir, exist_ok=True) # './data/iphone/apple/logs/iphone_fit_native_add3_20250904_151709/mosca' NOTE makedir mosca
    os.makedirs(log_dir, exist_ok=True) # './data/iphone/apple/logs/iphone_fit_native_add3_20250904_151709/mosca'

    if square_loss_flag: # False
        raise RuntimeError(
            "Square loss is not good! set [square_loss_flag] flag to false"
        )

    # * The small change is resp. to the init stage
    solid_mask = scf._node_certain # T,M
    solid_xyz = scf._node_xyz.clone().detach() # T,M,3

    optimizer = torch.optim.Adam(
        scf.get_optimizable_list(lr_np=lr_p, lr_nq=lr_q, lr_nsig=lr_sig) # p: point, q: quaternion, sig: sigma
    )
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        (total_steps - decay_start), # 1500 - 500
        eta_min=min(lr_p, lr_q) / decay_factor, # decay_factor: 30
    )
    ...
    scf.update_topology(curve_mask=solid_mask if use_mask_topo else None) # use_mask_topo: True, has curve_mask! NOTE update mosca edge & arap pair
    ...
    for step in tqdm(range(total_steps)):
        ...
        if step in update_all_topo_steps: # []
            ...
            scf.update_topology(curve_mask=None, verbose=True) # has curve_mask! NOTE update mosca edge & arap pair
        elif step % mlevel_resample_steps == 0 and step > 0: # 100
            if update_full_topo: # False
                scf.update_topology(
                    curve_mask=solid_mask if use_mask_topo else None, verbose=True
                ) # has curve_mask! NOTE update mosca edge & arap pair
            else:
        ...
        loss_coord, loss_len, loss_p_acc, loss_q_acc, loss_p_vel, loss_q_vel = (
            __compute_physical_losses__(
                ...
                max_time_window, # T_all + 1
                square=square_loss_flag, # False
            )
        )

        # loss of near original curve
        diff_to_solid_xyz = (scf._node_xyz - solid_xyz.detach()).norm(dim=-1) ** 2 # T,M
        ...
        loss.backward()
        if hard_fix_valid: # True
            scf.mask_xyz_grad(~solid_mask) # NOTE Preventing gradient transfer in invalid areas
        optimizer.step()

        # * learn scheduler
        if step > decay_start: # 500
            scheduler.step()
        # * viz and save .pth
        if step % 50 == 0:
        ...
        if step == 0 and viz_interval == -1:
            logging.info("warning! viz_interval = -1, change viz_interval to 600")
            viz_interval = 600
            
        if (step % viz_interval == 0 or step == total_steps - 1) and viz_interval > 0: # 600
            viz_frame = viz_curve(
            ...
            if viz_node_rgb is not None:
                ...
                viz_valid_color = torch.tensor([0.0, 1.0, 0.0]).to(solid_mask.device) # green
                viz_invalid_color = torch.tensor([1.0, 0.0, 0.0]).to(solid_mask.device) # red
                # T,N,3
                viz_mask_color = (
        ...
        if viz_debug_interval > 0 and step % viz_debug_interval == 0: # 600
    ...
    for plt_i, plt_pack in enumerate(
        ...
        plt.plot(plt_pack[1]), plt.title(plt_pack[0]), plt.yscale("log")
    plt.tight_layout()
    plt.savefig(osp.join(log_dir, f"{prefix}dynamic_scaffold_init.jpg")) # NOTE dynamic_scaffold_init.jpg
    plt.close()
    make_video_from_pattern(
        osp.join(viz_dir, f"{prefix}dyn_scf_init*.jpg"),
        osp.join(viz_dir, f"{prefix}dyn_scf_init.gif"),
    ) # NOTE dyn_scf_init_000001.jpg, dyn_scf_init.gif 
    ...
    if save_flag: # True
        ...
        imageio.mimsave(
            osp.join(log_dir, f"{prefix}dynamic_scaffold_init.gif"), # NOTE dynamic_scaffold_init.gif
            [(fr * 255).astype(np.uint8) for fr in viz_frame],
        )
        torch.save(
            scf.state_dict(), osp.join(log_dir, f"{prefix}dynamic_scaffold_init.pth") # NOTE dynamic_scaffold_init.pth
        )
