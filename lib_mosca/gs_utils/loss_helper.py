...
def compute_dep_loss(
    ...
    pred_dep = render_dict["dep"][0] # H,W
    target_dep = target_dep.detach() # H,W
    if st_invariant: # NOTE normal depth before compute loss, iphone: True
        prior_t = torch.median(target_dep[sup_mask > 0.5]) # mean
        pred_t = torch.median(pred_dep[sup_mask > 0.5])
        prior_s = (target_dep[sup_mask > 0.5] - prior_t).abs().mean() # variance
