...
class StaticGaussian(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.op_update_exclude = [] # what this?

        self.register_buffer("max_scale", torch.tensor(max_scale).squeeze()) # 0.03
        self.register_buffer("min_scale", torch.tensor(min_scale).squeeze()) # 0.0
        self.register_buffer("max_sph_order", torch.tensor(max_sph_order).squeeze()) # 3
        self._init_act(self.max_scale, self.min_scale)

        # * init the parameters
        self._xyz = nn.Parameter(torch.as_tensor(init_mean).float()) # NOTE init_mean
        if init_q is None:
        ...
        self._rotation = nn.Parameter(init_q) # NOTE init_q
        assert len(init_s) == len(init_mean)
        assert init_s.ndim == 2
        assert init_s.shape[1] == 3
        self._scaling = nn.Parameter(self.s_inv_act(init_s)) # NOTE init_s
        o = self.o_inv_act(init_o)
        self._opacity = nn.Parameter(o) # NOTE init_o
        sph_rest_dim = 3 * (sph_order2nfeat(self.max_sph_order) - 1) # 45 = 3 * (16 - 1)
        self._features_dc = nn.Parameter(RGB2SH(init_rgb)) # NOTE init_rgb
        self._features_rest = nn.Parameter(torch.zeros(self.N, sph_rest_dim)) # NOTE init_rgb
        if init_id is None: # semantic segmentation / instance segmentation
            init_id = torch.zeros(self.N, dtype=torch.int32) # NOTE all zero
        self.register_buffer("group_id", init_id) # NOTE group_id
    ...
    def _init_act(self, max_s_value, min_s_value):
        ...
        def s_act(x):
            ...
            return min_s_value + torch.sigmoid(x) * (max_s_value - min_s_value) # interesting
    ...
    def get_optimizable_list(
        ...
        lr_sph_rest = lr_sph / 20 if lr_sph_rest is None else lr_sph_rest # interesting
        l = [
