...
from se3_utils.se3_bspline import SE3BSplineUltraFast
...
class MoSca(nn.Module):
    def __init__(
        ...
        t_list=None,
        # min num of nodes
        min_node_num=20, # NOTE 32
        max_node_num=16384, # NOTE 16384
    ):
        # T,N,3; T,N,4
        super().__init__()

        # * xyz
        if node_grouping is None: # None TODO node_grouping is interesting
            node_grouping = torch.zeros_like(node_certain[0], dtype=torch.int)  # M
        if len(torch.unique(node_grouping)) > 1 and sort_node: # sort_node: False
        ...
        self._node_xyz = nn.Parameter(node_xyz.to(init_device)) # MARK: learned
        ...
        if spatial_unit_hard_set is None or spatial_unit_hard_set <= 0.0: # iphone: 0.02
            logging.info(
            ...
            spatial_unit = spatial_unit_hard_set # 0.02
        self.register_buffer("spatial_unit", torch.tensor(spatial_unit))
        self.register_buffer("skinning_k", torch.tensor(skinning_k).long())
        self.register_buffer("topo_dist_top_k", torch.tensor(topo_dist_top_k).long()) # 3
        self.register_buffer("topo_sample_T", torch.tensor(topo_sample_T).long()) # 100
        self.register_buffer("topo_th_ratio", torch.tensor(topo_th_ratio)) # 5.0
        self.register_buffer("fixed_topology_flag", torch.tensor(False)) # False
        self.set_multi_level(
            mlevel_arap_flag, mlevel_list, mlevel_k_list, mlevel_w_list
        )
        self.register_buffer("mlevel_detach_nn_flag", torch.tensor(mlevel_detach_nn_flag)) # True
        self.register_buffer("mlevel_detach_self_flag", torch.tensor(mlevel_detach_self_flag)) # False
        ...
        self.update_topology(curve_mask=self._node_certain) # has curve_mask

        # * compute optimal frame and init the rotation
        self.compute_rotation_from_xyz()

        # * RBF interpolation
        self.register_buffer("max_sigma", torch.tensor(sigma_max_ratio * spatial_unit)) # 10. * 0.02
        self.register_buffer(
            "init_sigma", torch.tensor(sigma_init_ratio * spatial_unit)
        ) # 5. * 0.02
        if node_sigma_logit is None: # None
            node_sigma_logit = self.sig_invact(torch.ones(self.M) * self.init_sigma) # M
        else:
            assert len(node_sigma_logit) == self.M
            assert node_sigma_logit.ndim <= 2
        if node_sigma_logit.ndim == 1:
            node_sigma_logit = node_sigma_logit[:, None] # M,1
        self._node_sigma_logit = nn.Parameter(node_sigma_logit) # MARK: learned 
        ...
        self.set_skinning_method() # setting self.__BLEND_FUNC__
        
        self.register_buffer("w_corr_maintain_sum_flag", torch.tensor(w_corr_maintain_sum_flag)) # False

        # * Buffers
        if t_list is None: # [0, 4, 8, ..., 468, 472, 474]
        ...
        self.min_node_num = min_node_num # 32
        self.max_node_num = max_node_num # 16384
    ...
    def compute_rotation_from_xyz(self):
        _node_quat = self.__compute_R_from_xyz__() # T,M,4
        if hasattr(self, "_node_rotation"):
            self._node_rotation.data = _node_quat
        else:
            self._node_rotation = nn.Parameter(_node_quat) # T,M,4 MARK: learned
    ...
    def init_control_state(self, control_state=None, sparse_init=False):

        if control_state is None:
            if sparse_init:
                control_state = torch.zeros((self.T, self.M), dtype=torch.bool, device=self._node_xyz.device)
                num_selected_t = (self.T + 1) // 2 # NOTE 50% T
                selected_t = torch.linspace(0, self.T - 1, steps=num_selected_t, device=self._node_xyz.device).round().long()
                control_state[selected_t] = True
            else:
                control_state = torch.ones((self.T, self.M), dtype=torch.bool, device=self._node_xyz.device)
        else:
            control_state = control_state.to(self._node_xyz.device)

        if "control_state" in self._buffers:
            self.control_state = control_state
        else:
            self.register_buffer("control_state", control_state)

    def compute_se3_bspline(self):

        R = q2R(self._node_rotation) # T,M,3,3
        t = self._node_xyz # T,M,3
        rt = torch.cat([R, t.unsqueeze(-1)], dim=-1)
        bottom = torch.zeros((self.T, self.M, 1, 4), device=R.device, dtype=R.dtype)
        bottom[..., 0, 3] = 1.0
        node_state_se3 = torch.cat([rt, bottom], dim=-2)
        # print("node number:", R.shape[1])

        self.se3_bspline = SE3BSplineUltraFast(node_state_se3, self.control_state)
        # print('se3 b-spline ok')

    def prune_control_points(self, prune_T=None):
        if prune_T is None:
            return None
        else:
            self.control_state[prune_T] = False # T,M
    ...
    def load_from_ckpt(
        ...
        if "control_state" in ckpt: # control_state is not in old ckpt
            scf.init_control_state()
    ...
    def set_multi_level(
        ...
        if self.mlevel_arap_flag: # True
            self.mlevel_list = mlevel_list # iphone: [1, 7, 15], [1, 6] 
            self.mlevel_k_list = mlevel_k_list # iphone: [16, 8, 8] [16, 8]
            self.mlevel_w_list = mlevel_w_list # iphone: [0.4, 0.3, 0.3] [0.4, 0.3]
            logging.info(
    ...
    def update_topology(
        ...
        start_t = time.time()
        if self.fixed_topology_flag: # False
            logging.warning(
            ...
            self.update_multilevel_arap_topo(verbose=verbose) # NOTE ARAP
            return # NOTE fixed_topology_flag return

        # compute curve _D_topo consider grouping
        unique_group_id = torch.unique(self._node_grouping) # tensor([0])
        if len(unique_group_id) == 1 or not self.break_topo_between_group: # iphone: True or not True
            self._D_topo = _compute_curve_topo_dist_( # M,M
                curve_xyz=self._node_xyz, # T,M,3
                curve_mask=curve_mask, # T,M
                top_k=self.topo_dist_top_k, # 3
                max_subsample_T=self.topo_sample_T, # 100
            ) # M,M
            torch.cuda.empty_cache()
        else: # keep topo between groups
        ...
        topo_dist, topo_ind = __compute_topo_ind_from_dist__(
            self._D_topo, self.skinning_k - 1 # 16 - 1
        ) # topo_dist: [M, skinning_k - 1], topo_ind: [M, skinning_k - 1]

        self_ind = torch.arange(self.M).to(self.device)
        topo_ind = torch.cat([self_ind[:, None], topo_ind], dim=1) # M, skinning_k
        topo_th = self.topo_th_ratio * self.spatial_unit # 5. * 0.02
        topo_mask = topo_dist < topo_th
        topo_mask = torch.cat([torch.ones_like(topo_mask[:, :1]), topo_mask], 1) # M, skinning_k
        self.topo_knn_ind = topo_ind # M, skinning_k
        self.topo_knn_mask = topo_mask # M, skinning_k

        if multilevel_update_flag: # iphone: True
            self.update_multilevel_arap_topo(verbose=verbose) # NOTE ARAP
        if verbose: # False
            logging.info(f"Topology updated in {time.time()-start_t:.2f}s")
        torch.cuda.empty_cache()
        return # self._D_topo, self.topo_knn_ind, self.topo_knn_mask,
    ...
    def update_multilevel_arap_topo(self, verbose=False):
        ...
        self.multilevel_arap_edge_list, self.multilevel_arap_dist_list = (
            __compute_multilevel_topo_ind_from_dist__(
                self._D_topo, # M,M
                K_list=self.mlevel_k_list, # [16,8,8]
                subsample_units=[self.spatial_unit * l for l in self.mlevel_list], # [1,7,15]
                verbose=verbose, # False
            )
        ) # self.multilevel_arap_edge_list: [[M*16, 2], [M*8, 2], [M*8, 2]], self.multilevel_arap_dist_list: [[M*16], [M*8], [M*8]]
        ...
        self.multilevel_arap_topo_w = multilevel_arap_topo_w # self.multilevel_arap_topo_w: [[M*16], [M*8], [M*8]]
        return # self.multilevel_arap_edge_list, self.multilevel_arap_dist_list, self.multilevel_arap_topo_w

    def warp(
        self,
        attach_node_ind, # N_dyn
        query_xyz, # N_dyn,3
        query_tid, # N_dyn
        target_tid,
        query_dir=None, # N_dyn,3,3
        skinning_w_corr=None, # None -> N_dyn,16
        dyn_o_flag=False, # True
    ):
        ...
        if isinstance(target_tid, int) or target_tid.ndim == 0:
            target_tid = target_tid * torch.ones_like(query_tid) # N_dyn

        # * identify skinning weights
        sk_ind, sk_w, sk_w_sum, sk_ref_node_xyz, sk_ref_node_quat = (
            self.get_skinning_weights(
                query_xyz=query_xyz, # N_dyn,3
                query_t=query_tid, # N_dyn
                attach_ind=attach_node_ind, # N_dyn
                skinning_weight_correction=skinning_w_corr, # None -> N_dyn,16
            )
        ) # [N_dyn,16] [N_dyn,16] [N_dyn,1] [N_dyn,16,3] [N_dyn,16,4]

        # * blending
        # sk_dst_node_xyz, sk_dst_node_quat = self.get_async_knns(target_tid, sk_ind) # [N_dyn,16,3] [N_dyn,16,4]
        sk_dst_node_xyz, sk_dst_node_quat = self.get_async_knns_se3_bspline(target_tid, sk_ind) # [N_dyn,16,3] [N_dyn,16,4]
        if dyn_o_flag: # True
            dyn_o = torch.clamp(sk_w_sum, min=0.0, max=1.0) # N_dyn, NOTE dynamic degree
        else:
            dyn_o = torch.ones_like(sk_w_sum) * (1.0 - DQ_EPS) # 0.001
        ret_xyz, ret_dir = self.__BLEND_FUNC__( # __BLEND_FUNC__: __DQB_warp__
            sk_w=sk_w, # [N_dyn,16]
            src_xyz=query_xyz, # [N_dyn,3]
            src_R=query_dir, # [N_dyn,3,3]
            sk_src_node_xyz=sk_ref_node_xyz, # [N_dyn,16,3]
            sk_src_node_quat=sk_ref_node_quat, # [N_dyn,16,4]
            sk_dst_node_xyz=sk_dst_node_xyz, # [N_dyn,16,3]
            sk_dst_node_quat=sk_dst_node_quat, # [N_dyn,16,4]
            dyn_o=dyn_o, # [N_dyn,1]
        ) # [N_dyn,3] [N_dyn,3,3]
    ...
    def get_async_knns(self, t, knn_ind): # t & knn_ind -> node_xyz & node_quat
        assert t.ndim == 1 and knn_ind.ndim == 2 and len(t) == len(knn_ind)
        # self._node_XXXX[t,knn_ind]
        with torch.no_grad():
            flat_sk_ind = t[:, None] * self.M + knn_ind # N,K
    ...
    def get_async_knns_se3_bspline(self, t, knn_ind): # t & knn_ind -> node_xyz & node_quat
        # query_t = time.time()
        sk_ref_node_xyz, sk_ref_node_quat = self.se3_bspline.query_poses(t, knn_ind) # t:[N], knn_ind:[N,K]
        # print(f"Query nodes in {time.time()-query_t:.2f}s for group {knn_ind.shape[-1]}")
        return sk_ref_node_xyz, sk_ref_node_quat # N,K,3; N,K,4
    ...
    def get_skinning_weights(
        ...
        query_t,
        attach_ind,
        skinning_weight_correction=None, # None -> N_dyn,16
    ):
        assert query_xyz.ndim == 2
        sk_ind = self.topo_knn_ind[attach_ind] # [N_dyn,16]
        sk_mask = self.topo_knn_mask[attach_ind] # [N_dyn,16] NOTE hard valid mask
        ...
        if isinstance(query_t, int) or query_t.ndim == 0:
            ...
            # sk_ref_node_xyz, sk_ref_node_quat = self.get_async_knns(query_t, sk_ind) # [N_dyn,16,3] [N_dyn,16,4]
            sk_ref_node_xyz, sk_ref_node_quat = self.get_async_knns_se3_bspline(query_t, sk_ind) # [N_dyn,16,3] [N_dyn,16,4]

        sq_dist_to_sk_node = (query_xyz[:, None, :] - sk_ref_node_xyz) ** 2 # [N_dyn,16,3] NOTE local node coordinate system
        sq_dist_to_sk_node = sq_dist_to_sk_node.sum(-1)  # [N_dyn,16]
        sk_w_un = (
            torch.exp(
                -sq_dist_to_sk_node / (2 * (self.node_sigma.squeeze(-1) ** 2)[sk_ind])
            )
            + 1e-6
        )  # [N_dyn,16] NOTE positive number

        sk_w_un = sk_w_un * sk_mask.float() # NOTE processing by hard valid mask
        if skinning_weight_correction is not None: # None
            assert len(skinning_weight_correction) == len(query_xyz)
            assert skinning_weight_correction.shape[1] == self.skinning_k
            if self.w_corr_maintain_sum_flag: # False
                tmp_sk_w_sum = abs(sk_w_un).sum(-1)[:, None] # [N_dyn,1]
                sk_w_un = abs(sk_w_un + skinning_weight_correction)
                new_sk_w_sum = torch.clamp(abs(sk_w_un).sum(-1), min=1e-6)[:, None] # [N_dyn,1]
                factor = tmp_sk_w_sum / new_sk_w_sum
                sk_w_un = sk_w_un * factor
            else:
                sk_w_un = abs(sk_w_un + skinning_weight_correction)
        sk_w_sum = sk_w_un.sum(-1) # N_dyn
        sk_w = sk_w_un / torch.clamp(sk_w_sum, min=1e-6)[:, None] # [N_dyn,16] NOTE softmax
    ...
    @torch.no_grad()
    def resample_node(self, resample_factor=1.0, use_mask=False, \
                      resample_ind=None, spatial_scale_min=2.0, spatial_scale_max=1.0): # 2.0 0.5
        old_M = self.M # ~ 20000
        if resample_ind is None: # None
            resample_ind = resample_curve(
                ...
                sample_margin=resample_factor * self.spatial_unit * spatial_scale_max, # 1. * 0.02 NOTE Downsample
                mask=self._node_certain, # T,M
                verbose=True,
            ) # shape: new_M, [new_ind1, new_ind2, ...] ~ 3000
        if len(resample_ind) < self.min_node_num: # 32
            # logging.warning(
            ...
            resample_ind = resample_curve(
                ...
                sample_margin=resample_factor * self.spatial_unit * spatial_scale_min, # 1. * 0.02 NOTE: Downsample
                mask=self._node_certain, # T,M
                verbose=True,
            )
        ...
        with torch.no_grad():
            ...
            if len(self._node_grouping.unique()) < len(self.unique_grouping): # if group is reduced
                logging.warning(
    ...
    def append_nodes_pnt(
        ...
        start_t = time.time()
        while len(new_node_xyz) > 0: # Amazing Iterative Method
            # identify the nearest chunk_size nodes to append and remove it from the new_node attrs
            existing_node_xyz = self._node_xyz[new_tid] # M,3
            dist_sq, nn_exsiting_ind, _ = knn_points(
                new_node_xyz[None], existing_node_xyz[None], K=1
            ) # [1,N,1], [1,N,1]
            dist_sq = dist_sq[0, :, 0] # N
            nearest_ind = torch.argsort(dist_sq)[:chunk_size] # chunk_size: 512
            nearest_mask = torch.zeros(len(new_node_xyz), dtype=bool)
            nearest_mask[nearest_ind] = True
            working_node_xyz = new_node_xyz[nearest_mask] # chunk_size NOTE new node nearest exist node
            working_node_quat = new_node_quat[nearest_mask] # chunk_size NOTE new node nearest exist node
            new_node_xyz = new_node_xyz[~nearest_mask] # N-chunk_size NOTE downsample new node
            new_node_quat = new_node_quat[~nearest_mask] # N-chunk_size NOTE downsample new node
            nn_exsiting_ind = nn_exsiting_ind[0, nearest_ind, 0] # chunk_size
            new_group_id = self._node_grouping[nn_exsiting_ind] # chunk_size
            ...
            for _t in tqdm(range(self.T)):
                ...
                nearest_node_ind = nearest_node_ind[0, :, 0] # chunk_size
            ...
            to_append_node_xyz = torch.stack(node_xyz_list, 0) # T,chunk_size,3
            to_append_node_quat = torch.stack(node_quat_list, 0) # T,chunk_size,4
            self.append_nodes_traj( # NOTE add traj
    ...
    def append_nodes_traj(
        ...
        # NOTE ADD CONTROL STATE
        rand_idx = torch.randint(0, self.control_state.size(0), (new_node_xyz_traj.shape[1],)) # M_new
        append_control_state = self.control_state[:, rand_idx] # T,M_new
        self.control_state = torch.cat([self.control_state, append_control_state], dim=1)
    ...
    def remove_nodes(self, optimizer, node_prune_mask):
        ...
        # NOTE PRUNE CONTROL STATE
        self.control_state = self.control_state[:, ~node_prune_mask]
    ...
    def compute_arap_loss(
        self,
        tids=None,
        temporal_diff_weight=[0.75, 0.25], # [0.6 0.4 0.3]
        temporal_diff_shift=[1, 4], # [2 8 16]
        ...
        if tids is None:
            tids = torch.arange(self.T).to(self.device)
        assert tids.max() <= self.T - 1
        xyz = self._node_xyz[tids] # T,M,3
        R_wi = q2R(self._node_rotation[tids]) # T,M,3,3
        topo_ind = self.topo_knn_ind # M,16
        topo_w = self.topo_knn_mask.float() # M,16 NOTE binary mask
        topo_w = topo_w / (topo_w.sum(dim=-1, keepdim=True) + 1e-6) # M,16 NOTE normalize
        local_coord = get_local_coord(xyz, topo_ind, R_wi) # T,N,K,3

        if detach_tids_mask is not None: # None
        ...
        loss_coord, loss_len, _, _ = compute_arap(
            local_coord,
            topo_w,
            temporal_diff_shift, # [2 8 16]
            temporal_diff_weight, # [0.6 0.4 0.3]
            reduce=reduce_type, # 'sum'
            square=square, # False
        )
    ...
    def get_optimizable_list(
        ...
        ret = []
        if lr_np is not None: # iphone mosca: 0.0001, iphone fit: 0.0003
            ret.append({"params": [self._node_xyz], "lr": lr_np, "name": "node_xyz"})
        if lr_nq is not None: # iphone mosca: 0.0001, iphone fit: 0.0003
            ret.append(
                {"params": [self._node_rotation], "lr": lr_nq, "name": "node_rotation"}
            )
        if lr_nsig is not None: # iphone mosca: 0.00001, iphone fit: 0.003
            ret.append(
                {
    ...
    @torch.no_grad()
    def mask_xyz_grad(self, mask): # mask: invalid region
    ...
    def __compute_R_from_xyz__(self):
        init_node_R = torch.eye(3).to(self.device)[None].repeat(self.M, 1, 1) # M,3,3
        new_R_list = [init_node_R]
        nn_ind, nn_mask = self.topo_knn_ind, self.topo_knn_mask # M,K
        for new_tid in tqdm(range(1, self.T)): # NOTE loop from 1, the first time node rotation is torch.eye(3)
            # ! do to the first frame!
            src_xyz, dst_xyz = self._node_xyz[new_tid - 1], self._node_xyz[new_tid] # M,3
            ...
            src_p = src_xyz[nn_ind] - src_xyz[:, None] # M,K,3 NOTE local coordinate system
            dst_q = dst_xyz[nn_ind] - dst_xyz[:, None]
            w = nn_mask
            w = w / w.sum(dim=-1, keepdim=True)  # M,K

            src_centroid = (src_p * w[..., None]).sum(1) # M,3 NOTE compute center
            dst_centroid = (dst_q * w[..., None]).sum(1)
            src_p = src_p - src_centroid[:, None] # M,K,3 NOTE centralization
            dst_q = dst_q - dst_centroid[:, None]

            # ! warning, dst = R_star @ src
            # dst @ dst.T = R_star @ src @ dst.T
            # R_star.T @ dst @ dst.T = src @ dst.T
            # R_star.T @ dst @ dst.T = W
            W = torch.einsum("nki,nkj->nkij", src_p, dst_q) # M,K,3,3
            W = W * w[:, :, None, None] # M,K,3,3
            W = W.sum(1) # M,3,3

            U, s, V = torch.svd(W.double()) # MARK: Deformation Principle
        ...
        new_R_list = torch.stack(new_R_list, dim=0) # T,M,3,3
        new_q_list = matrix_to_quaternion(new_R_list) # T,M,4
        return new_q_list
    ...
    def resample_time(self, new_tids, new_node_certain, mode="linear"):
        ...
        for new_t in tqdm(new_tids): 
            ...
            elif mode == "linear": # iphone: linear, nvidia_marble: linear
        ...
        self._node_xyz.data = new_xyz_list # T_all,N_dyn_filter_downsample,3
        self._node_rotation.data = new_quat_list # T_all,N_dyn_filter_downsample,4
        assert new_node_certain.shape[1] == self.M and len(new_tids) == len(
            new_node_certain
        )
        self._node_certain.data = new_node_certain # T_all,N_dyn_filter_downsample

        # update t_list
        remap = [torch.where(new_tids == t)[0][0].item() for t in self._t_list.cpu()] # [0, 4, 8, ..., 468, 472, 474]
        self._t_list = new_tids.to(self._t_list)
        return remap  # new index in the new list


def __compute_delta_Rt_ji__(R_wi, t_wi, R_wj, t_wj): # MARK: Deformation Principle
    # R: N,K,3,3; t: N,K,3
    ...
    # p_t=j_world = (R_wj @ R_wi.T) @ p_t=i_world + (t_wj - (R_wj @ R_wi.T) @ t_wi)
    assert R_wi.ndim == 4 and R_wi.shape[2:] == (3, 3)  # source R 
    assert t_wi.ndim == 3 and t_wi.shape[2] == 3        # source T
    assert R_wj.ndim == 4 and R_wj.shape[2:] == (3, 3)  # target R
    assert t_wj.ndim == 3 and t_wj.shape[2] == 3        # target T
...
def __DQB_warp__(
    sk_w, # N,K
    src_xyz, # N,3
    sk_src_node_xyz, # N,K,3
    sk_src_node_quat, # N,K,4
    ...
    sk_R_tq, sk_t_tq = __compute_delta_Rt_ji__(
        R_wj=q2R(sk_dst_node_quat), # [N,K,3,3]
        t_wj=sk_dst_node_xyz, # [N,K,3]
        R_wi=q2R(sk_src_node_quat), # [N,K,3,3]
        t_wi=sk_src_node_xyz, # [N,K,3]
    ) # [N,K,3,3] [N,K,3]
    sk_dq_tq = Rt2dq(sk_R_tq, sk_t_tq) # N,K,8
    # * Dual Quaternion skinning
    dq = torch.einsum("nki,nk->ni", sk_dq_tq, sk_w) # N,8
    # use dyn mask to blend a unit dq into
    unit_dq = torch.Tensor([1, 0, 0, 0, 0, 0, 0, 0]).to(sk_w)[None].expand(len(dq), -1) # N,8; static dq
    dq = dq * dyn_o[:, None] + unit_dq * (1 - dyn_o)[:, None] # N,8 NOTE dyn_o: dynamic degree
    with torch.no_grad():
        assert dq.max() < 1e6, f"{dq.max()}"
    dq = dq2unitdq(dq) # N,8
    R_tq, t_tq = dq2Rt(dq) # N,3,3; N,3
    # * apply the transformation to the leaf
    mu_dst = torch.einsum("nij,nj->ni", R_tq, src_xyz) + t_tq # NOTE local node coordinate system
    if src_R is not None:
        fr_dst = torch.einsum("nij,njk->nik", R_tq, src_R) # NOTE local node coordinate system
    else:
        fr_dst = None
...
def _compute_curve_topo_dist_(
    ...
    ind_pair = torch.triu_indices(N, N).permute(1, 0)  # P,2; P = N*(N-1)/2 + N = N*(N+1)/2
    P = len(ind_pair)
    cur = 0
    ...
    while cur < P:
        ...
        d = robust_curve_dist_kernel(src, dst, src_m, dst_m, top_k) # chunk

        flat_D = torch.cat([flat_D, d]) # P
        cur = cur + chunk
    # convert the list to upper and lower triangle
    _d = torch.zeros(N, N).to(xyz) # N,N
...
def __compute_multilevel_topo_ind_from_dist__(
    ...
    torch.cuda.empty_cache()
    assert (
        not shrink_level # NOTE shrink_level useless
    ...
    N, _ = dist.shape # N: track number
    current_set = torch.arange(N, device=dist.device)

    edge_list, dist_list = [], []
    for k, unit in zip(K_list, subsample_units): # K_list: [16 8 8]
        torch.cuda.empty_cache()
        if not shrink_level: # not False
        ...
        cur_D = dist[current_set][:, current_set] # M,M
        resample_ind = resample_curve(cur_D, unit, mask=None, verbose=verbose) # new_N NOTE downsample curve
        ...
        sub_D = cur_D[
            :, resample_ind
        ] # ! before convert to global ind, first subsample the sub_D; N,new_N
        resample_ind = current_set[resample_ind].clone() # new_N; useless

        # for all previous set curve, find the K nearest subset curve
        nearest_ind = torch.topk( # N,K
            sub_D, min(k, len(resample_ind)), dim=1, largest=False
        ).indices
        nearest_ind = resample_ind[nearest_ind] # N,K
        src_ind = current_set[:, None].expand(-1, min(k, len(resample_ind))) # N,K
        edge = torch.stack([src_ind, nearest_ind], dim=-1).reshape(-1, 2) # N,K,2 -> N*K,2
...
def resample_curve(D, sample_margin, mask=None, verbose=False):
    N = D.shape[0] # N,N
    assert D.shape == (N, N)
    if mask is None: # None or _node_certain
        rank_inds = torch.randperm(N) # N
    else:
        rank_inds = torch.argsort(mask.sum(0), descending=True) # N
    sampled_inds = rank_inds[:1] # torch.size([1])
    ...
    # sort sampled_inds
    sampled_inds = torch.sort(sampled_inds).values # N_new
    if verbose:
        logging.info(
