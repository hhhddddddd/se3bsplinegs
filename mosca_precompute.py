...
def get_moca_processor(pre_cfg):
    moca_processor = MoCaPrep(
        ...
        ),  # "depthcrafter", "metric3d", "uni"; iphone: "sensor"
        tap_mode=getattr(
            pre_cfg, "tap_mode", "bootstapir"
        ),  # "spatracker", "cotracker"; iphone: "bootstapir"
        flow_mode=getattr(pre_cfg, "flow_mode", "raft"), # iphone: "raft"
        align_metric_flag=getattr(pre_cfg, "align_metric_flag", True), # iphone: True
    )
    return moca_processor


def load_imgs_from_dir(src):
    img_dir = osp.join(src, "images") # './data/iphone/apple/images'
    img_fns = sorted(
        [it for it in os.listdir(img_dir) if it.endswith(".png") or it.endswith(".jpg")]
    ) # only training images
    img_list = [imageio.imread(osp.join(img_dir, it))[..., :3] for it in img_fns] # 255
    return img_list, img_fns

def load_imgs_from_nvidia_dir(src):
    with open(os.path.join(src, "splits/train.json")) as f: # train.json
        split_dict = json.load(f)
    img_fns = split_dict["frame_names"] # only training images
    img_fns = [it + ".png" for it in img_fns]
    img_dir = osp.join(src, "rgb/2x") # './data/nvidia_marbles/balloon1/rgb/2x'
    img_list = [imageio.imread(osp.join(img_dir, it))[..., :3] for it in img_fns] # 255
    return img_list, img_fns
...
def preprocess(
    ...
    BOUNDARY_EHNAHCE_TH = getattr(pre_cfg, "boundary_enhance_th", -1) # nvidia_marble: -1
    DEPTH_DIR_POSTFIX = "_depth_sharp" if BOUNDARY_EHNAHCE_TH > 0 else "_depth" # nvidia_marble: "_depth"

    EPI_TH = getattr(pre_cfg, "epi_th", 1e-3) # nvidia_marble: 0.001
    DEPTH_BOUNDARY_TH = getattr(
        pre_cfg, "depth_boundary_th", 1.0
    )  # this is in the median=1.0 space; nvidia_marble: 1.0

    TAP_CHUNK_SIZE = getattr(pre_cfg, "tap_chunk_size", 5000) # 5000
    ...
    if not resample_for_dynamic: # not True
    ...
    s2d = (
        ...
        .load_epi()
        .load_dep(f"{moca_processor.dep_mode}{DEPTH_DIR_POSTFIX}", DEPTH_BOUNDARY_TH, nvidia_mode) # metric3d_depth
        .normalize_depth(median_depth=1.0)
        .recompute_dep_mask(depth_boundary_th=DEPTH_BOUNDARY_TH)
        .load_track(f"*uniform*{moca_processor.tap_mode}", min_valid_cnt=4) # NOTE uniform
        .load_vos() # video object segment
    )

    if hasattr(s2d, "epi"): # NOTE True
        sample_mask = s2d.epi > EPI_TH # dynamic mask
    else:
    ...
    resampling_mask_dilate_ksize = getattr(pre_cfg, "resampling_mask_dilate_ksize", 7) # iphone: 7, nvidia_marble: 7
    sample_mask = (
    ...
    if nvidia_mode:
        moca_processor.compute_tap(
            ...
            mask_list=sample_mask.detach().cpu().numpy() > 0, # dynamic region
            dep_list=moca_processor.load_dep_list_nvidia(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip_dynamic_resample", action="store_true", help="skip dynamic resample")
    parser.add_argument("--nvidia_mode", action="store_true", help="use nvidia settings")
    args, unknown = parser.parse_known_args()

    if args.nvidia_mode: 
        img_list, img_fns = load_imgs_from_nvidia_dir(args.ws)
    else:
    ...
    preprocess(
        ...
        pre_cfg=prep_cfg,
        resample_for_dynamic=not args.skip_dynamic_resample, # iphone: not False
        nvidia_mode=args.nvidia_mode, # iphone: False
    )
