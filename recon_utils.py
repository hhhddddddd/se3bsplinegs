...
def setup_recon_ws(ws, fit_cfg, logdir="logs", exp_name="useless_exp"):
    seed_everything(SEED)
    # get datetime
    now = datetime.now()
    dt_string = now.strftime("%Y%m%d_%H%M%S") # year month day hour minute second
    ...
    backup_dir = osp.join(log_path, "src_backup") # backup the code
...
def update_s2d_track_identification(
    ...
    if s2d.has_epi:
        ...
        with torch.no_grad():
            ...
            epi_error_list = epi_error_list.squeeze(-1).cpu() # T,N
    else:
        epi_data = np.load(osp.join(log_path, "tracker_epi.npz")) # NOTE load
    ...
    if photo_error_masks is not None: # iphone: None
