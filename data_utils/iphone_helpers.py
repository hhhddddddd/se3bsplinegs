...
def load_iphone_cameras(dir_path):
    ...
    test_dir_path = dir_path.replace('cameras','test_cameras') if not dir_path.endswith('test_cameras') else dir_path
    test_all_fns = [f.split("_")[1].split(".")[0] for f in os.listdir(test_dir_path) if f.startswith("1_")]
    test_all_fns.sort()
    ...
    for cam_id in all_cam_ids:
        ...
        for fn in tqdm(cam_fns):
            ...
            if fn.split("_")[1].split(".")[0] in test_all_fns and dir_path.endswith('cameras') and False:
                with open(osp.join(test_dir_path, fn.replace('0_','1_')), "r") as f:
            ...
            T_wi = np.eye(4) # NOTE wi <=> c2w
            t_wi = np.asarray(cam_data["position"]) # t_wi in c2w NOTE cam_data["position"] c2w
            R_wi = np.asarray(cam_data["orientation"]).T # R_wi in c2w NOTE cam_data["orientation"] w2c
            T_wi[:3, :3] = R_wi
            T_wi[:3, 3] = t_wi
            # world = R.T @ local
            ...
            fovdeg = np.rad2deg(2 * np.arctan(short_size / (2 * focal))) # NOTE why short_size
...
def load_iphone_gt_poses(src, t_subsample): # iphone: t_subsample == 1
    ...
    gt_training_cams = {k: v[::t_subsample] for k, v in gt_training_cams[0].items()} # only subsample
    ...
    gt_training_cam_T_wi = gt_training_cams["T_wi"] # T, 4, 4 NOTE c2w
    # ! this bound which test frames can be retrieved
    gt_training_tids = gt_training_cams["t"] # T [0, 1, 2, .., 474, 475]
    gt_training_cxcy_ratio = gt_training_cams["cxcy_ratio"] # T

    # NOTE filter test camera
...
def load_nvidia_cameras(dir_path, train=True):

    if train:
        all_fns = glob.glob(osp.join(dir_path, "*.json"))
        all_fns = [os.path.basename(f) for f in all_fns if os.path.basename(f).startswith("0_")]
        all_fns.sort()
    else:
        all_fns = glob.glob(osp.join(dir_path, "*.json"))
        all_fns = [os.path.basename(f) for f in all_fns if not os.path.basename(f).startswith("0_")]
        all_fns.sort()
    all_cam_ids = np.array([int(f.split("_")[0].split(".")[0]) for f in all_fns])
    all_cam_ids = np.unique(all_cam_ids).tolist()
    logging.info(f"Loading iphone cameras from {dir_path} with camid={all_cam_ids}")
    ...
    for cam_id in all_cam_ids:
        ...
        for fn in tqdm(cam_fns):
            ...
            focal = cam_data["focal_length"] # 1x resolution
            T_wi = np.eye(4) # NOTE wi <=> c2w
            t_wi = np.asarray(cam_data["position"]) # t_wi in c2w NOTE cam_data["position"] c2w
            R_wi = np.asarray(cam_data["orientation"]).T # R_wi in c2w NOTE cam_data["orientation"] w2c
            T_wi[:3, :3] = R_wi
            ...
            # ! get the fov in current code format, the focal with the shortest side
            short_size = min(cam_data["image_size"])  # 1x resolution
            fovdeg = np.rad2deg(2 * np.arctan(short_size / (2 * focal))) # NOTE why short_size

            # also load camera center
            principal_point = np.asarray(cam_data["principal_point"]) # 1x resolution
            image_size = np.asarray(cam_data["image_size"]) # 1x resolution
            cx_ratio = principal_point[0] / image_size[0]
            cy_ratio = principal_point[1] / image_size[1]
            cxcy_ratio_list.append([cx_ratio, cy_ratio])
...
def load_nvidia_gt_poses(src, t_subsample): # iphone: t_subsample == 1
    gt_training_cams = load_nvidia_cameras(osp.join(src, "camera"), train=True)
    assert (
        len(gt_training_cams) == 1
    ), "Only support Mono camera for now in nvidia marbles dataset"
    # subsample the gt camera
    gt_training_cams = {k: v[::t_subsample] for k, v in gt_training_cams[0].items()} # only subsample
    gt_testing_cams = load_nvidia_cameras(osp.join(src, "camera"), train=False)
    gt_training_fov = gt_training_cams["fovdeg"][0]
    logging.info(
        f"Iphone gt training camera fov is {gt_training_cams['fovdeg'][0]:.3f} deg."
    )
    ...
    gt_testing_fov_list, gt_testing_fns_list = [], []
    gt_testing_cxcy_ratio_list = []
    for it in gt_testing_cams.values(): # NOTE select test camera according to time
        sample_index = [i for i, t in enumerate(it["t"]) if t in gt_training_tids]
        gt_testing_tids = [gt_training_tids.index(it["t"][i]) for i in sample_index]
        gt_testing_tids_list.append(gt_testing_tids)
        gt_testing_cam_T_wi = it["T_wi"][sample_index]
