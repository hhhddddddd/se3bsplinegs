...
def align_ate_c2b_use_a2b(traj_a, traj_b, traj_c=None):
    ...
    traj_a = traj_a.float().cpu().numpy() # gt_train_camera
    traj_b = traj_b.float().cpu().numpy() # optimized_train_camera
    traj_c = traj_c.float().cpu().numpy() # gt_test_camera
    ...
    s, R, t = alignTrajectory(t_a, t_b, quat_a, quat_b, method="sim3") # NOTE important
...
def alignTrajectory(p_es, p_gt, q_es, q_gt, method, n_aligned=-1):
    ...
    """
    assert p_es.shape[1] == 3 # t_a
    assert p_gt.shape[1] == 3 # t_b
    assert q_es.shape[1] == 4 # quat_a
    assert q_gt.shape[1] == 4 # quat_b
