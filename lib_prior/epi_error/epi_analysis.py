...
def viz_epi_error_folder(epi_list, save_fn):
    ...
    save_fn = save_fn.replace('.mp4', '.gif')
...
def analyze_epi(working_dir, step_list=[1], num_threads=None, nvidia_mode=False):
    start_t = time.time()
    if nvidia_mode:
        img_fn_list = sorted(glob.glob(os.path.join(working_dir, "rgb/2x", "0_*")))
    else:
