...
class MoCaPrep:
    def __init__(
        ...
        self.viz_flag = viz_flag # True
        self.device = torch.device(device) # cuda
        self.seed = seed # 12345

        self.__init_flow__(flow_mode, device) # demo: raft; iphone: raft

        self.__init_depth__( # get depth model
            dep_mode, # demo: depthcrafter; iphone: sensor; nvidia: metric3d
            device,
            depth_exclude_sky=depth_exclude_sky, # False
            metric3d_depth_version=metric3d_depth_version, # giant2
            align_metric_flag=align_metric_flag, # True
            align_metric_model=align_metric_model, # metric3d
        )
        self.__init_tap__( # get tap model
            tap_mode, # demo: spatracker; iphone: bootstapir; nvidia: cotracker;
            device,
            spatracker_S_lenth,
            cotrakcer_version,
    ...
    def create_workspace(
        self, save_dir, t_list, fn_list, img_list, nvidia_mode=False, imgdirname="images"
    ):
        # create workspace
        os.makedirs(save_dir, exist_ok=True)
        # imageio.mimsave(osp.join(save_dir, "input.mp4"), img_list)
        imageio.mimsave(osp.join(save_dir, "input.gif"), img_list)
        configure_logging(osp.join(save_dir, "preprocess.log"), debug=False)
        if nvidia_mode:
            images_dir = osp.join(save_dir, "rgb/2x")
        else:
    ...
    def compute_depth(
        ...
        if not hasattr(self, "depth_model"): # NOTE iphone: False
        ...
        if self.depth_exclude_sky: # False
        ...
        if self.dep_mode == "metric3d":
            ...
            self.depth_process_func(
                ...
                invalid_mask_list=invalid_mask_list, # None
                n_steps=depthcrafter_denoising_steps, # 25
            )
        else:
            self.depth_process_func(
                self.depth_model,
        ...
        if enhance_depth_boudary_th > 0: # 1.0
            assert (
                enhance_depth_boudary_open_ksize >= 3
            ...
            imageio.mimsave(
                osp.join(
                    save_dir,
                    f"sharper_boundary_mask_th={enhance_depth_boudary_th}_ks={enhance_depth_boudary_ksize}_oks={enhance_depth_boudary_open_ksize}.gif",
                ),
    ...
    def compute_flow(
        self, save_dir, fn_list, img_list, step_list=[1], epi_num_threads=16, nvidia_mode=False
    ):
        start_t = time.time()
        ...
        analyze_epi(save_dir, num_threads=epi_num_threads, step_list=step_list, nvidia_mode=nvidia_mode)
        return
    ...
    def process( # MARK: process
        ...
        ########################################################################
        img_name_list = [osp.basename(fn) for fn in img_name_list] # 0_00000.png
        img_list = np.asarray(img_list) # 50, 480, 854, 3

        ########################################################################
        # * 0. Init
        self.create_workspace(save_dir, t_list, img_name_list, img_list, nvidia_mode)
        # ########################################################################
        # * 2. Depth
        self.compute_depth(
            save_dir,
        ...
        if self.flow_mode == "raft" and compute_flow: # True
            logging.info(f"Generating Flow and Analysis EPI")
            self.compute_flow(
                save_dir,
                img_name_list,
                img_list,
                step_list=flow_steps, # [1 3]
                epi_num_threads=epi_num_threads, # 64
        ...
        # * 4. Long-track
        if compute_tap: # True
            dep_name = self.dep_mode + "_depth" # demo: depthcrafter_depth; iphone: sensor_depth
            ...
            if nvidia_mode:
                dep_list = self.load_dep_list_nvidia(save_dir, dep_name) # 'depthcrafter_depth_sharp'
            else:
                dep_list = self.load_dep_list(save_dir, dep_name) # 'depthcrafter_depth_sharp'
            uniform_sample_list = np.ones_like(dep_list) > 0
            self.compute_tap(
                ws=save_dir,
                save_name=f"uniform_dep={self.dep_mode}",
                n_track=n_track,
                img_list=img_list, # T,H,W,3
                mask_list=uniform_sample_list, # all region (dynamic & static)
                dep_list=dep_list, # T,H,W
                K=known_camera_K,
    ...
    def load_dep_list_nvidia(self, ws, name=None):
        if name is None:
            name = f"{self.dep_mode}_depth"
        dep_dir = osp.join(ws, name)
        if osp.isdir(dep_dir):
            ...
            dep_dir = osp.join(ws, 'depth/2x')
            dep_fns = sorted([osp.join(dep_dir, fn) for fn in os.listdir(dep_dir)])
            dep_list = np.stack([np.load(fn) for fn in dep_fns], 0)
        return dep_list
