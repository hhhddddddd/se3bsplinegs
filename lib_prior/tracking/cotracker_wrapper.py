...
def get_cotracker(device, online_flag=True, cotrakcer_version="3"):
    if online_flag: # True
        cotracker = torch.hub.load(
            "facebookresearch/co-tracker", f"cotracker{cotrakcer_version}_online" # cotracker3_online
        ).to(device)
        # cotracker = torch.hub.load("/home/xuankai/.cache/torch/hub/co-tracker-main", f"cotracker{cotrakcer_version}_online", pretrain=False, \
        #                     source='local', model_path='/home/xuankai/.cache/torch/hub/checkpoints/scaled_online.pth').to(device)
    else:
...
def cotracker_process_folder(
    ...
    imageio.mimsave(
        osp.join(viz_dir, f"{save_name}_sample_mask.gif"),
    ...
    for round in range(num_slice):
        ...
        imageio.mimsave(
            osp.join(viz_dir, f"{save_name}_r={round}_quries.gif"), viz_list
        )
        if online_flag:
