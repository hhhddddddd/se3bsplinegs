...
class DepthCrafterDemo:
    def __init__(
        ...
        # )
        unet = DiffusersUNetSpatioTemporalConditionModelDepthCrafter.from_pretrained(
            "/home/xuankai/dataset/xuankai/checkpoint-sd/depthcrafter",
            low_cpu_mem_usage=True,
            torch_dtype=torch.float16,
            local_files_only=local_files_only,
        )
        # load weights of other components from the provided checkpoint
        self.pipe = DepthCrafterPipeline.from_pretrained(
            "/home/xuankai/dataset/xuankai/checkpoint-sd/stable-video-diffusion-img2vid-xt",
            unet=unet,
            torch_dtype=torch.float16,
            variant="fp16",
            local_files_only=local_files_only,
...
def depthcrafter_process_folder(
    ...
    T, H, W, C = img_list.shape # 50, 480, 854, 3

    # reshape to 64 base size
    new_H = H // 64 * 64 # 448
    new_W = W // 64 * 64 # 832
    # use opencv to resize
    working_img_list = [] # 50, 448, 832, 3
    for img in img_list:
