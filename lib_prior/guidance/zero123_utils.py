...
class Zero123(nn.Module):
    ...
    def get_cam_embeddings(self, elevation, azimuth, radius, default_elevation=0):
        if self.use_stable_zero123: # False
    ...
    def train_step(self, pred_rgb, elevation, azimuth, radius, view_ind, step_ratio=None, guidance_scale=5, \
                   as_latent=False, default_elevation=0, timesteps=None, clip_loss=False):
        # pred_rgb: tensor [1, 3, H, W] in [0, 1]

        batch_size = pred_rgb.shape[0] # 1

        if as_latent: # False
            latents = F.interpolate(pred_rgb, (32, 32), mode='bilinear', align_corners=False) * 2 - 1
        else:
            pred_rgb_256 = F.interpolate(pred_rgb, (256, 256), mode='bilinear', align_corners=False) # 1,3,256,256
            latents = self.encode_imgs(pred_rgb_256.to(self.dtype)) # 1,4,32,32
        ...
        w = (1 - self.alphas[t]).view(batch_size, 1, 1, 1) # 1,1,1,1

        with torch.no_grad():
            noise = torch.randn_like(latents) # 1,4,32,32
            latents_noisy = self.scheduler.add_noise(latents, noise, t) # 1,4,32,32

            x_in = torch.cat([latents_noisy] * 2) # 2,4,32,32
            t_in = torch.cat([t] * 2) # 2

            T = self.get_cam_embeddings(elevation, azimuth, radius, default_elevation) # 1,1,4
            if timesteps is not None: # None
                ...
                cc_emb = torch.cat([self.embeddings[0][view_ind].unsqueeze(0).unsqueeze(0), T], dim=-1) # 1,1,772 (768+4=772)
            ...
            if timesteps is not None: # None
                ...
                vae_emb = self.embeddings[1][view_ind].unsqueeze(0) # 1,4,32,32
        ...
        noise_pred_cond, noise_pred_uncond = noise_pred.chunk(2) # [1,4,32,32] [1,4,32,32]
        noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond) # 1,4,32,32

        grad = w * (noise_pred - noise) # 1,4,32,32
        grad = torch.nan_to_num(grad) # 1,4,32,32

        target = (latents - grad).detach() # 1,4,32,32
        loss = 0.5 * F.mse_loss(latents.float(), target, reduction='sum') # strange: big loss ~600 -> ~160 
