...
def gather_track_from_buffer(track, track_base_mask, buffer_list):
    ...
    if buffer_list.ndim == 3:
        buffer_list = buffer_list.unsqueeze(3) # T, H, W, 1
    T, N = track_base_mask.shape
    # fill the ret with nan
    ret = torch.zeros(T, N, buffer_list.shape[-1], dtype=buffer_list.dtype) # T, N, 1
    ret = ret.to(track.device)
    ret.fill_(np.nan) # NOTE True NaN
...
def filter_track(track, track_vis, dep_mask_list, min_valid_cnt=4):
    ...
    track_dep_mask, _ = gather_track_from_buffer( # T, N, 1
        track[..., :2].long(), track_vis, dep_mask_list
    )
    track_mask = track_dep_mask.squeeze(-1) * track_vis # T, N
    # check whether a track is visible and has valid depth more more thant min_valid_cnt times
    valid_cnt = track_mask.sum(0) # N
...
def laplacian_filter_depth(depths, threshold_ratio=0.5, ksize=5, open_ksize=3):
    ...
    for dep in depths:
        ...
        mask, error = detect_depth_occlusion_boundaries(dep, threshold, ksize) # process sensor depth
        mask = mask > 0.5
...
def get_homo_coordinate_map(H, W): # get homogeneous coordinate map
...
class Saved2D(nn.Module):
    def __init__(self, ws, nvidia_mode) -> None:
        super().__init__()
        self.ws = ws

        # this init only load the RGBs and build basic attrs
        if nvidia_mode:
            self.load_rgb_nvidia("rgb/2x")
        else:
    ...
    def load_zero123(self):
        
        # NOTE ckpt path
        self.guidance_zero123 = Zero123('cuda', model_key='./checkpoints/zero123-xl-diffusers')
        logging.info("loaded zero123!")

        self.fg_imgs = self.rgb * self.dyn_mask[..., None] # T,H,W,3
        self.fg_imgs_512 = F.interpolate(self.fg_imgs.permute(0, 3, 1, 2), (256, 256), \
                                     mode='bilinear', align_corners=False).to('cuda') # T,3,resize_H,resize_W

        with torch.no_grad():

            c_list, v_list = [], []
            for i in tqdm(range(self.T), desc="Loading Zero123 embeddings"): # T
                c, v = self.guidance_zero123.get_img_embeds(self.fg_imgs_512[i][None]) # [1,768], [1,4,32,32]  
                c_list.append(c)
                v_list.append(v)
            self.guidance_zero123.embeddings = [torch.cat(c_list, 0), torch.cat(v_list, 0)] # [[T,768], [T,4,32,32]]
        
        del self.fg_imgs, self.fg_imgs_512, c_list, v_list
    ...
    def load_rgb(self, rgb_dirname="images"):
        ...
        if osp.exists(img_npz):
            ...
            img_times = [f.split('_')[-1] for f in img_names]
        ...
        self.real_names = img_fns
        images = images[..., :3]
        self.register_gradfree_buffer("rgb", images.detach())
        return self
    
    @torch.no_grad
    def load_rgb_nvidia(self, rgb_dirname="rgb/2x"):
        with open(os.path.join(self.ws, "splits/train.json")) as f: # train.json
            split_dict = json.load(f)
        img_fns = split_dict["frame_names"] # only training images
        img_dir = osp.join(self.ws, rgb_dirname) # './data/nvidia_marbles/balloon1/rgb/2x'

        img_names = img_fns
        images = [imageio.imread(osp.join(img_dir, img_fn + ".png")) for img_fn in img_fns]
        images = torch.Tensor(np.stack(images)) / 255.0  # T,H,W,3
    ...
    def load_dep(
        self,
        depth_dirname="depth_metric3d", # iphone: sensor_depth
        ...
        if nvidia_mode:
            dep_dir = osp.join(self.ws, 'depth/2x')
        else:
            dep_dir = osp.join(self.ws, depth_dirname)
        if osp.isdir(dep_dir):
            if nvidia_mode:
                dep = [
                    np.load(osp.join(dep_dir, f"{img_name}.npy")) 
            ...
            dep = np.stack(dep)  # T,H,W NOTE depth, not disparity
        else:
            dep_fn = dep_dir + ".npz"
            assert osp.exists(dep_fn)
            dep = np.load(dep_fn)["dep"]
        if mask_depth_flag: # True, NOTE depth mask
            dep_mask, _ = laplacian_filter_depth(dep, depth_boundary_th, 5) # T,H,W
            logging.info(f"Dep Boundary Mask {dep_mask.mean()*100:.2f}%")
            dep_mask = dep_mask * (dep > depth_min) * (dep < depth_max) # depth_min: 0.001 depth_max: 1000
            dep = np.clip(dep, depth_min, depth_max) # Force change of depth range
        else:
    ...
    def load_track(self, track_npz_key="spatracker", min_valid_cnt=4):
        ...
        long_track_fns = glob(osp.join(self.ws, f"*{track_npz_key}*.npz")) # uniform / dynamic
        ...
        for track_fn in long_track_fns:
            ...
            track.append(track_data["tracks"]) # T, N_track, 2
            track_mask.append(track_data["visibility"]) # T, N_track
        # ! explicitly round to long
        track = torch.from_numpy(np.concatenate(track, 1)).float()  # T,N,2/3; uniform + dynamic
        ...
        if min_valid_cnt > 0: # iphone: 4, nvidia_marbles: 4, nvidia: 1
        ...
        if self.track.shape[-1] == 3 and hasattr(self, "scale_nw"):
            ...
            self.track[:, :, 2] = self.track[:, :, 2].clone() * self.scale_nw # TODO i think only chang z is error
            
        return self 

    @torch.no_grad()
    def load_vos(self, vos_dirname="vos_deva/Annotations"): # video object segmentation
    ...
    def load_flow(self, flow_dirname="flow_raft"):
        ...
        flow_dir = osp.join(self.ws, flow_dirname) # './data/iphone/apple/flow_raft'
        if not osp.exists(flow_dir):
            ...
            return self
        for fn in tqdm(sorted(os.listdir(flow_dir)),desc="Loading Flow"): # iphone: [-3 -1 +1 +3]
            if not fn.endswith(".npz"):
                continue
            flow_fn = osp.join(flow_dir, fn)
            flow_data = np.load(flow_fn, allow_pickle=True)
            flow, mask = flow_data["flow"], flow_data["mask"] # flow: H,W,2; mask: H,W
            src_t, dst_t = fn[:-4].split("_to_") # 0_00000.png, 0_00001.png
            src_t = self.frame_names.index(src_t[:-4]) # 0
            dst_t = self.frame_names.index(dst_t[:-4]) # 1
        ...
        flow_list = np.stack(flow_list, 0) # N,H,W,2
        flow_mask_list = np.stack(flow_mask_list, 0) # N,H,W
        src_t_list = np.array(src_t_list) # N
        dst_t_list = np.array(dst_t_list) # N
        ...
        self.flow_ij_to_listind_dict = flow_ij_to_listind_dict # N
        return self

    @torch.no_grad()
    def register_2d_identification(self, static_2d_mask, dynamic_2d_mask): # H,W
    ...
    @torch.no_grad()
    def register_track_indentification(self, static_track_mask, dynamic_track_mask): # only track
    ...
    def get_mask_by_key(self, key):
        if key == "all":
            return torch.ones_like(self.dep_mask) # T,H,W
        elif key == "static":
    ...
    def rescale_depth(self, dep_scale):
        ...
        self.dep = self.dep.clone() * dep_scale[:, None, None] # T,H,W
    ...
    def rescale_perframe_depth_from_bundle(self, bundle_pth_fn=None):
        ...
        dep_scale = bundle_data["dep_scale"] # T; iphone: all one, nvidia_marbles: all one, nvidia_rodynrf: varying
        logging.info(f"Rescale depth with {bundle_pth_fn} and scale={dep_scale}")
        self.rescale_depth(dep_scale)
        return self

    @torch.no_grad()
    def normalize_depth(self, median_depth: float = 1.0): # iphone: don't normalize
        # align the median of the depth to 1.0
        if median_depth < 0: # nvidia: 1.0
            scale_nw = 1.0
        else:
            world_depth = self.dep.clone()
            world_depth_fg = world_depth[self.dep_mask]
            world_depth_median = torch.median(world_depth_fg) # torch.size([])
            scale_nw = (
        ...
        self.dep = self.dep.clone() * scale_nw # NOTE depth scale
        if hasattr(self, "track") and self.track.shape[-1] == 3:
            logging.info(f"Also align the 3D track with the depth scale")
            self.track[:, :, 2] = self.track[:, :, 2].clone() * scale_nw # TODO only chang z is error
        return self 
