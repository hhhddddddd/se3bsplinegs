...
def render_cam_pcl(
    ...
    S = torch.zeros_like(frame) # N_all,3,3
    S[:, 0, 0] = scale[:, 0]
    S[:, 1, 1] = scale[:, 1]
    S[:, 2, 2] = scale[:, 2]
    actual_covariance = frame @ (S**2) @ frame.permute(0, 2, 1) # N_all,3,3
    ...
    camera_center = viewmatrix.inverse()[3, :3] # w2c

    if len(bg_color) < 6: # True
        bg_color = bg_color + [0.0] * (6 - len(bg_color)) # [random, random,random, 0.0, 0.0, 0.0]
    ...
    rotations = None
    # JH
    cov3D_precomp = strip_lowerdiag(actual_covariance) # N,3,3 -> N,6
    ...
    # P_w = Frame @ P_local
    dir_local = torch.einsum("nji,nj->ni", frame, dir_cam)  # frame.T <=> c2w -> w2c NOTE camera coordinate system -> gaussian coordinate system
    dir_local = F.normalize(
        dir_local, dim=-1
    )  # If frame is not SO(3) but Affinity, have to normalize
    N = len(color_feat) # gaussian number
    shs_view = color_feat.reshape(N, -1, 3) # N, Deg, Channels
    _deg = shs_view.shape[1] # 16
    if _deg == 1:
        active_sph_order = 0
    elif _deg == 4:
        active_sph_order = 1
    ...
    if add_buffer is None:
        add_buffer = torch.zeros_like(colors_precomp) # N,3
    assert add_buffer.shape == colors_precomp.shape
    colors_precomp = torch.cat([colors_precomp, add_buffer], -1) # N,6
    ...
    if center_handling_flag: # do not understand
    ...
    ret["dep"] = ret["dep"] / torch.clamp(ret["alpha"], 1e-6, 1.0) # what this? NOTE depth scale
    return ret
