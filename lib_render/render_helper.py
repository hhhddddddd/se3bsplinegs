...
def render(
    ...
    if torch.is_tensor(gs_param[0]):  # direct 5 tuple
        ...
        for i in range(1, len(gs_param)): # NOTE form 1
            mu = torch.cat([mu, gs_param[i][0]], 0) # !Q: global (static) cat local (dynamic) !A: global (static) cat global (dynamic)
            fr = torch.cat([fr, gs_param[i][1]], 0) # !Q: global (static) cat local (dynamic)
            s = torch.cat([s, gs_param[i][2]], 0)
            o = torch.cat([o, gs_param[i][3]], 0)
            sph = torch.cat([sph, gs_param[i][4]], 0)
    if opa_replace is not None: # None
        assert isinstance(opa_replace, float)
        o = torch.ones_like(o) * opa_replace
    s = s * scale_factor # 1.0

    # cvt to cam frame cvt: convert
    assert T_cw.ndim == 2 and T_cw.shape[1] == T_cw.shape[0] == 4
    R_cw, t_cw = T_cw[:3, :3], T_cw[:3, 3] # NOTE w2c
    mu_cam = torch.einsum("ij,nj->ni", R_cw, mu) + t_cw[None]
    fr_cam = torch.einsum("ij,njk->nik", R_cw, fr)

    # render
    render_dict = render_cam_pcl(
        mu_cam, # NOTE camera coordinate system
        fr_cam, # NOTE camera coordinate system
        s,
        o,
        sph,
        H,
    ...
    if bg_cache_dict is not None: # None
